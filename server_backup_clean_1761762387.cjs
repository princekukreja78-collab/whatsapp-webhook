...
// --- /prompt endpoint: returns a { reply, type } response for webhook ---
app.post('/prompt', async (req, res) => {
  try {
    const { from, message } = req.body || {};
    // load CRM config from DB_FILE
    const db = fs.existsSync(DB_FILE) ? JSON.parse(fs.readFileSync(DB_FILE, 'utf8')) : {};
    const systemPrompt = db.system_prompt || "You are Signature Savings assistant. Be professional and concise.";
    const replySignature = db.reply_signature || '';

    // If Hugging Face is configured, call it
    const HF_KEY = process.env.HF_API_KEY || process.env.HF_KEY || '';
    const HF_MODEL = process.env.HF_MODEL || process.env.HF_MODEL_NAME || '';
    if (HF_KEY && HF_MODEL) {
      ...
      try {
        ...
        let text = '';
        if (Array.isArray(hfJson) && hfJson[0] && hfJson[0].generated_text) text = hfJson[0].generated_text;
        else if (hfJson.generated_text) text = hfJson.generated_text;
        else if (typeof hfJson === 'string') text = hfJson;
        else if (hfJson?.error) text = replySignature || `Sorry, AI error: ${hfJson.error}`;
        else text = JSON.stringify(hfJson).slice(0, 2000);

        const finalReply = (text && text.trim()) ? text.trim() : (replySignature || 'Sorry, AI is not available at the moment.');
        return res.json({ reply: finalReply, type: 'text' });
      } catch (hfErr) {
        console.error('/prompt hf call failed', hfErr);
        const fallback = replySignature || 'Sorry, AI is not available at the moment.';
        return res.json({ reply: fallback, type: 'text' });
      }
    }

    // No HF configured — return stored reply_signature or fallback
    const fallbackReply = replySignature || 'Sorry, AI is not available at the moment.';
    return res.json({ reply: fallbackReply, type: 'text' });
  } catch (err) {
    console.error('/prompt error', err);
    return res.status(500).json({ error: 'prompt_error' });
  }
});
// --- /prompt endpoint: returns a { reply, type } response for webhook ---
app.post('/prompt', async (req, res) => {
