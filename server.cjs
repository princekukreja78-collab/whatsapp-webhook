...
async function waSendRaw(payload) {
  try { console.log("WA_SEND_ATTEMPT payload_preview:", JSON.stringify(payload).slice(0,800)); } catch(e){}
...
async function waSendText(to, body) {
  try { console.log("WA_SEND_TEXT_CALL to=", to, " body_preview=", String(body).slice(0,300)); } catch(e){}
  try {
    const payload = {
...
async function loadPricing() {
  try {
    ...
    console.log('✅ Pricing loaded for:', Object.keys(tables));
  } catch (e) {
...
try {
    ...
    console.log('📩 Message from', from, ':', text?.slice(0,140));
