...
async function waForwardImage(to, mediaId, caption = "") {
  const url = `https://graph.facebook.com/v21.0/${(_config.currentPhoneNumberId && _config.currentPhoneNumberId()) || _config.PHONE_NUMBER_ID}/messages`;
...
// sendSheetWelcomeTemplate_OLD removed — had undefined `img` bug, superseded by server.cjs version
...
try {
  ...
  router.get('/api/leads/export-csv', async (req, res) => {
    try {
      let leads = [];
      if (_config.getAllLeads) {
        try { leads = await _config.getAllLeads(); } catch(e) { leads = []; }
      }
...
// getMediaUrl was defined here but never exported, so server.cjs passed null to
// the webhook and every vision fault analysis died on "is not a function".
module.exports = { init, router, getMediaUrl };
