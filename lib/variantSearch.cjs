...
function stripBudgetWords(query) {
  return String(query || '')
    ...
    // A place name is where to register, not a model to match. Leaving it in
    // made "budget 25 lakh in noida" hunt for a car called Noida and return
    // nothing, when the state is picked from the same words anyway.
    .replace(/\b(delhi|new delhi|ncr|noida|greater noida|ghaziabad|gurgaon|gurugram|faridabad|haryana|uttar pradesh|u\.?p\.?|himachal|shimla|h\.?p\.?|chandigarh|punjab)\b/gi, ' ')
    // Generic words people wrap a budget in — none of them name a model
    .replace(/\b(cars?|vehicles?|show|me|any|some|best|good|options?|available|list|suggest|need|want|looking|for|a|an|the)\b/gi, ' ')
    .replace(/\b\d{4,8}\b/g, ' ')
    .trim();
}
...
async function search({ query, city = 'Delhi', limit = 14 }) {
  // "suv under 15 lakh" names a shape, not a model — the sheets have no body
  // column, so the model registry answers it.
  const bodies = require('./bodyTypes.cjs');
  const wantBody = bodies.askedBody(query);
  const budget = parseBudget(query);
  const modelPart = budget ? stripBudgetWords(query) : query;
  const q = norm(modelPart);
  // norm() strips spaces, so tokenise the raw text and normalise each word.
  const qTokens = String(modelPart || '')
    .toLowerCase()
    .split(/[^a-z0-9()+]+/)
    .map((w) => norm(w))
    .filter((w) => w.length > 1);
  // A pure budget query ("under 15 lakh") has no model left to match on
  let budgetOnly = budget > 0 && q.length < 3;
  if (bodies.askedBody(query)) budgetOnly = true;   // the shape is the filter
  if ((!q && !budget && !bodies.askedBody(query)) || typeof _config.loadPricingFromSheets !== 'function') {
  ...
  for (const [brand, tb] of Object.entries(tables || {})) {
    ...
    for (const row of tb.data) {
      ...
      if (wantBody && !bodies.isBody(`${make} ${model}`, wantBody)) continue;
      // "creta sx diesel" only matched when those words sat together in that
      // order. Every word must appear, order and spacing are the customer's
      // business, not ours.
      if (!budgetOnly && !wantBody && q && !qTokens.every((tk) => hay.includes(tk))) continue;
  ...
  // "VX(O)" contains "VX", so a search for the plain trim listed the dearer (O)
  // cars alongside it — same fault the bot had, and the two engines have to
  // agree. Only show them when the search actually asks for them.
  // Read the tokens off the RAW query: norm() strips spaces as well as
  // punctuation, so "hycross vx" arrives here as "hycrossvx" with nothing to
  // split on.
  const rawQ = String(modelPart || '');
  const askedForO = /\(\s*o\s*\)|\b[a-z]{1,4}\s*o\b|\b[a-z]{1,4}o\b/i.test(rawQ);
  const trimTokens = rawQ.toLowerCase().split(/[^a-z0-9]+/).filter(tk => /^[a-z]{1,4}$/.test(tk));
  if (!askedForO && trimTokens.length) {
    const plainOnly = out.filter(c => !trimTokens.some(t =>
      new RegExp('\\b' + t + '\\s*(\\(\\s*o\\s*\\)|o\\b)', 'i').test(String(c.variant || ''))));
    if (plainOnly.length && plainOnly.length < out.length) out = plainOnly;
  }
...
// parseBudget/stripBudgetWords/pickState are shared with the WhatsApp bot so a
// budget typed there resolves exactly as it does on the site.
module.exports = { init, router, search, statePrices, parseBudget, stripBudgetWords, pickState };
