...
async function handleSalesmanReply(from, msgText, imageUrl) {
  ...
  // Store this reply — save images via mediaStore if available
  if (imageUrl) {
    let savedUrl = imageUrl;
    if (_config.mediaStore && imageUrl.startsWith('wamid')) {
      // imageUrl is actually a media ID — download and store
      const localUrl = await _config.mediaStore.saveMedia(imageUrl, enquiry.carModel, from, enquiry.id);
      if (localUrl) savedUrl = localUrl;
    }
    enquiry._replies.push({ type: 'image', url: savedUrl });
    console.log(`Enquiry ${enquiry.id}: dealer sent image → ${savedUrl}`);
  }
  if (msgText) {
...
async function receiveDeal(dealData) {
  ...
  const dealMsg =
    ...
    `${bestPrice}\n\n` +
    `_Negotiated from our dealer network._\n\n` +
    `---\n` +
    `*Why Buy Through MR. CAR?*\n\n` +
    `1. Complete *Car History Report* upon finalization\n` +
    `2. *Car Warranty* available (T&C & charges apply)\n` +
    `3. *Loan Options* — Vanilla, Bullet, Step-Up/Step-Down with foreclosure benefits\n` +
    `4. Systematic *Insurance Renewal* follow-ups\n` +
    `5. Free *RC Transfer* assistance\n` +
    `6. *1 Year Road Side Assistance (RSA)*\n\n` +
    `---\n` +
    `Reply *BOOK* to proceed or *NEW DEAL* for another enquiry.\n\n` +
    `_MR. CAR — Your Trusted Car Buying Partner_`;
