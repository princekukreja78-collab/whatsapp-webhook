...
async function search(opts = {}) {
  ...
  const pages = opts.pages || 3; // fetch 3 pages for more results
  const results = [];

  // Model-specific URL gives best results (ignores city but we filter later)
  // CarWale uses hyphens not spaces: "land-rover", not "land rover"
  const slugify = (s) => (s || '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');

  let baseUrl;
  if (opts.brand && opts.model) {
    baseUrl = `https://www.carwale.com/used/${slugify(opts.brand)}-${slugify(opts.model)}/`;
  } else if (opts.model) {
    const brand = opts.brand || '';
    if (brand) {
      baseUrl = `https://www.carwale.com/used/${slugify(brand)}-${slugify(opts.model)}/`;
    } else {
      baseUrl = `https://www.carwale.com/used/cars-in-${city}/`;
    }
  } else {
    baseUrl = `https://www.carwale.com/used/cars-in-${city}/`;
  }
