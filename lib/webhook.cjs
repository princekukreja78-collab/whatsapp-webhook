...
function _parseUsedCarQuery(text) {
  ...
  const result = { model: '', city: '', minBudget: 0, maxBudget: 0 };
  ...
  let cleaned = t
    .replace(/\b(used|pre[-\s]?owned|preowned|second[-\s]?hand|car|cars|around|about|near|under|below|within|upto|up to|max|budget|approx|lakh|lac|in)\b/gi, '')
      ...
      if (explicitUsed || hasYear || lastSvc === 'USED') {
        ...
        if (_config.carSearch) {
          try {
            ...
            if (parsed.model) {
              await _config.waSendText(from, `Searching *${parsed.model}* across our dealer network & verified listings...`);
              ...
              if (results.length > 0) {
                const msg = _config.carSearch.formatForWhatsApp(results, stats, { model: parsed.model, city: parsed.city || 'delhi' });
                await _config.waSendText(from, msg);
