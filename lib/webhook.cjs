...
// ==================================================
// GROUP / MUTED NUMBER CHECK — SILENT OBSERVER
// Bot stays completely silent, only ingests car data
// ==================================================
// Groups: always silent
if (_config.groupIngest && _config.groupIngest.isGroupOrMuted(from)) {
  await _config.groupIngest.handleGroupMessage(from, msgText, name, type, msg?.image?.id || null);
  return res.sendStatus(200); // NEVER reply to groups
}

// Dealers: silent ingest if car listing, normal reply if enquiry/greeting
if (_config.groupIngest && _config.groupIngest.isDealerNumber(from)) {
  const ingested = await _config.groupIngest.handleDealerMessage(from, msgText, name);
  if (ingested) return res.sendStatus(200); // Car listing ingested, stay silent
  // Not a car listing — fall through to normal bot flow
}
