...
global.__WA_MSG_LOCK__.add(dedupKey);
    ...
    try {
      ...
      if (!selectedId && msgText && global.panIndiaPrompt && global.panIndiaPrompt.get(from)) {
        ...
        if (/^(compare|compare states?|compare state wise|state ?wise|state prices?|pan[\s-]?india)\b/i.test(t)) {
          selectedId = 'BTN_PANIND_YES';
        } else if (/^(breakup|break ?up|price breakup|on[\s-]?road breakup|breakdown)\b/i.test(t)) {
          selectedId = 'BTN_PRICE_BREAKUP';
        }
        // Hand it over as a button press and nothing else. The smart-intent
        // engine runs long before the button handler, and any wording it didn't
        // recognise as a button title made it DELETE the quote context and then
        // read "compare" as a two-car comparison — "tell me the two models to
        // compare" instead of the state-wise prices.
        if (selectedId) msgText = '';
      }
    } catch (e) {
  ...
  console.warn("Smart intent engine failed:", e?.message || e);
    ...
    if (selectedId) {
      ...
      if (selectedId === 'BTN_PANIND_YES') {
        ...
        out.push('');
        out.push('_Reply *BREAKUP* for the on-road split of this variant._');
        await _config.waSendText(from, out.join('\n'));
        // Context used to be dropped here, so asking for the breakup right after
        // a comparison answered "I could not retrieve the variant".
        _config.setLastService(from, 'PAN_INDIA_PROMPT');
        return res.sendStatus(200);
      }
