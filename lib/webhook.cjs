...
// A photo on its own is an upload (intake, RC card, price sheet). The AI
// preliminary check only runs when the sender asks for it in words — a fault,
// inspection or paint question typed with the photo.
const PHOTO_CHECK_RE = /\b(check|checkup|inspect|inspection|analysis|analyse|analyze|evaluate|evaluation|report|issue|problem|fault|defect|damage|damaged|dent|scratch|leak|leakage|noise|smoke|crack|broken|rust|repaint|repainted|paint|painted|accident|condition|ppf|coating)\b/i;
function _wantsPhotoCheck(text) {
  const t = String(text || '').trim();
  if (!t) return false;
  return PHOTO_CHECK_RE.test(t);
}

// A lead is a person, not a message. Fourteen intake photos used to raise
// fourteen "New Lead Received" alerts on the admin's phone.
const _leadAlertAt = new Map();
const LEAD_ALERT_COOLDOWN_MS = 10 * 60 * 1000;
function _leadAlertDue(from) {
  const at = _leadAlertAt.get(from);
  if (at && Date.now() - at < LEAD_ALERT_COOLDOWN_MS) return false;
  _leadAlertAt.set(from, Date.now());
  return true;
}

// One preliminary check per burst — ten captioned photos are one question.
const _photoCheckAt = new Map();
const PHOTO_CHECK_COOLDOWN_MS = 90 * 1000;
function _photoCheckOnCooldown(from) {
  const at = _photoCheckAt.get(from);
  return !!(at && Date.now() - at < PHOTO_CHECK_COOLDOWN_MS);
}
function _markPhotoCheck(from) { _photoCheckAt.set(from, Date.now()); }

// Parse used car query: "Creta around 12 lakh" → { model: "creta", minBudget: 10, maxBudget: 14 }
function _parseUsedCarQuery(text) {
  const t = (text || '').toLowerCase().trim();
  const result = { model: '', city: '', minBudget: 0, maxBudget: 0, panIndia: false, fuel: '', maxAge: 0 };
  ...
  else if (/\b(hybrid|hev|phev)\b/i.test(t)) result.fuel = 'hybrid';
  else if (/\b(electric|ev|bev)\b/i.test(t)) result.fuel = 'electric';
  else if (/\b(cng)\b/i.test(t)) result.fuel = 'cng';
  else if (/\b(both)\b/i.test(t)) result.fuel = 'both';

  // Detect max age: "under 3 years", "within 5 years", "less than 3 year"
  const ageMatch = t.match(/(?:under|within|less than|max|upto|up to)\s*(\d+)\s*(?:year|yr|sal)/i);
  if (ageMatch) result.maxAge = Number(ageMatch[1]);
  ...
  // Extract model: remove known noise words, fuel, age, budget phrases
  let cleaned = t
    ...
    .replace(/\b(diesel|dsl|petrol|pet|hybrid|hev|phev|electric|ev|bev|cng|both)\b/gi, '')
    .replace(/\b(year|years|yr|yrs|sal|old)\b/gi, '')
...
router.post('/admin/test_alert', async (req, res) => {
  try {
    ...
    const resp = await fetch(
      `https://graph.facebook.com/v19.0/${(_config.currentPhoneNumberId && _config.currentPhoneNumberId()) || process.env.PHONE_NUMBER_ID}/messages`,
      {
        method: "POST",
        headers: {
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    // --- PHONE NUMBER ID CHECK: only process messages meant for THIS bot ---
    // One service, one inventory, but more than one number may point at it:
    // list the others in ALT_PHONE_NUMBER_IDS and the staging number is served
    // here too, with everything landing in the same store. Replies go back out
    // through whichever number the message arrived on.
    // entry[0].id is the WhatsApp Business Account id — the only place it is
    // handed to us, and what template management is posted to.
    try {
      const wabaId = req.body?.entry?.[0]?.id;
      if (wabaId && global.__wabaId !== wabaId) {
        global.__wabaId = String(wabaId);
        console.log('Webhook: WhatsApp Business Account id =', global.__wabaId);
      }
    } catch (e) {}

    const webhookPhoneId = value.metadata?.phone_number_id;
    const acceptedNumberIds = [String(_config.PHONE_NUMBER_ID || '').trim()]
      .concat(String(process.env.ALT_PHONE_NUMBER_IDS || '').split(',').map(x => x.trim()))
      .filter(Boolean);
    if (webhookPhoneId && acceptedNumberIds.length && !acceptedNumberIds.includes(String(webhookPhoneId))) {
      if (_config.DEBUG) console.log('Webhook: ignoring message for number', webhookPhoneId);
      return res.sendStatus(200);
    }
    if (webhookPhoneId && _config.setOutboundNumberId) _config.setOutboundNumberId(String(webhookPhoneId));

    // The broadcast number is not the shop's front door. It exists to send
    // dealer stock alerts, so the only things it answers for are the ones that
    // protect the number: an opt-out, and getting a real question in front of a
    // human. Everything else the customer bot does stays on the main number.
    const broadcastId = String(process.env.BROADCAST_PHONE_NUMBER_ID || '').trim();
    // Not for the owner: he forwards leads and runs commands on whichever number
    // is to hand, and a generic "someone will call you" is not an answer to that.
    const fromAdmin = !!(_config.inventory && _config.inventory.isDealAdmin &&
      _config.inventory.isDealAdmin(value.messages?.[0]?.from));
    if (broadcastId && !fromAdmin && String(webhookPhoneId || '') === broadcastId) {
      try {
        const bMsg = value.messages?.[0];
        const bFrom = bMsg?.from;
        const bText = String(bMsg?.text?.body || bMsg?.interactive?.button_reply?.title || '').trim();
        if (bFrom && bText) {
          const optOut = /^\s*(stop|unsubscribe|stop promotions|band karo|band karein|mat bhejo|remove me|do not send)\b/i.test(bText);
          if (optOut) {
            if (_config.followUp && typeof _config.followUp.stopFollowUp === 'function') _config.followUp.stopFollowUp(bFrom);
            if (_config.suppress && typeof _config.suppress.add === 'function') _config.suppress.add(bFrom, 'opted out on the broadcast number');
            console.log(`Broadcast: ${bFrom} opted out`);
            await _config.waSendText(bFrom, 'Done — you will not receive stock alerts from us again. Reply START if you change your mind.');
          } else if (/^\s*start\b/i.test(bText)) {
            if (_config.suppress && typeof _config.suppress.remove === 'function') _config.suppress.remove(bFrom);
            await _config.waSendText(bFrom, 'You are back on the stock alerts list.');
          } else {
            // A dealer replying to a stock alert is a warm lead — hand it to a
            // human rather than letting the quote engine answer on this number.
            if (typeof _config.sendAdminAlert === 'function') {
              _config.sendAdminAlert({ from: bFrom, name: '', text: `💬 Reply on the broadcast number from ${bFrom}: ${bText.slice(0, 300)}` }).catch(() => {});
            }
            await _config.waSendText(bFrom, 'Thanks — someone from MR. CAR will call you shortly. For full stock, specs and EMI: https://www.vehyra.in/cars');
          }
        }
      } catch (e) {
        console.warn('Broadcast number handling failed', e && e.message ? e.message : e);
      }
      return res.sendStatus(200);
    }

    // --- STAGING MODE: restrict who gets full service ---
    // Photo ingest, insurance coord, enquiry routing work for everyone
    // Full menu/quotes only for admin + testers
    const _isStagingMode = _config.STAGING_MODE === 'true' || _config.STAGING_MODE === true;
    const _testerList = (_config.TESTER_NUMBERS || '').split(',').map(s => s.trim()).filter(Boolean);
    const _isAllowedOnStaging = !_isStagingMode ||
      (value.messages?.[0]?.from === _config.ADMIN_WA) ||
      _testerList.includes(value.messages?.[0]?.from || '');
    ...
    try {
      ...
      if (msg && msg.from) {
        ...
        try {
          // Photos being ingested as a listing already reach the admin as the
          // draft — forwarding each one as well is fourteen duplicates.
          const partOfIntake = !!(_config.photoIngest &&
            ((typeof _config.photoIngest.isCollecting === 'function' && _config.photoIngest.isCollecting(senderForAuto)) ||
             _config.photoIngest.hasPending(senderForAuto))) ||
            !!(_config.inventory && _config.inventory.isDealAdmin && _config.inventory.isDealAdmin(senderForAuto));
          if (!partOfIntake && msg.type === "image" && msg.image?.id && _config.ADMIN_WA && senderForAuto !== _config.ADMIN_WA) {
        ...
        // ---- 2. AI VISION: only when the customer ASKS for a photo check ----
        // This used to run on EVERY image "to test the pipeline", so a dealer
        // uploading eight intake photos got eight preliminary-check replies on
        // top of the intake questionnaire. It now needs an explicit ask, never
        // fires on an intake/ingest upload, and answers a burst only once.
        try {
          if (msg.type === "image" && msg.image?.id) {
            const caption = msg.image?.caption || "";
            const combinedText = `${lastMsgForAuto || ""} ${caption || ""}`.toLowerCase();

            const askedForCheck = _wantsPhotoCheck(combinedText);
            const isIngestUpload =
              (_config.inventory && _config.inventory.isDealAdmin(senderForAuto)) ||
              (_config.photoIngest && _config.photoIngest.hasPending(senderForAuto)) ||
              (_config.groupIngest && (_config.groupIngest.isGroupOrMuted(senderForAuto) ||
                                       _config.groupIngest.isDealerNumber(senderForAuto)));
            const onCooldown = _photoCheckOnCooldown(senderForAuto);
            ...
            if (askedForCheck && !isIngestUpload && !onCooldown) {
              const mediaUrl = await _config.getMediaUrl(msg.image.id);
              if (mediaUrl) {
                _markPhotoCheck(senderForAuto);
        ...
        // Whatever they typed IS the search — keep it against the lead, so the
        // desk opens the call with what the customer was actually looking at.
        try {
          if (lastMsgForAuto && typeof _config.noteLeadInterest === 'function'
              && String(lastMsgForAuto).trim().length > 2) {
            _config.noteLeadInterest(senderForAuto, `Asked: ${String(lastMsgForAuto).trim()}`);
          }
        } catch (e) {}
...
await _config.pushLeadToGoogleSheet({
  ...
  lead_type: 'whatsapp_query'
        ...
        try {
          if (_config.ADMIN_WA && senderForAuto !== _config.ADMIN_WA && _leadAlertDue(senderForAuto)) {
            const what = lastMsgForAuto
              || (msg.type === 'image' ? 'sent photos' : msg.type === 'text' ? 'No text' : `sent ${msg.type}`);
            const body =
              `🔔 *New Lead Received*\n\n` +
              `👤 Name: ${senderNameForAuto}\n` +
              `📱 Phone: ${senderForAuto}\n` +
...
global.__WA_MSG_LOCK__.add(dedupKey);
    ...
    try {
      if (type === 'text' && msg.text && typeof msg.text.body === 'string') {
        ...
        // Template quick-reply taps arrive as type "button" with a payload
        selectedId = msg.button.payload || msg.button.text || null;
      ...
      // Typing the words has to work as well as tapping the button. The compare
      // and breakup options only ever existed as buttons, so a customer who
      // scrolled past them — or whose client didn't render them — had no way
      // back to a quote they had already been given.
      if (!selectedId && msgText && global.panIndiaPrompt && global.panIndiaPrompt.get(from)) {
        const t = msgText.trim();
        // The compare and breakup actions stay BUTTON-ONLY. Typing the words
        // only brings the buttons back — but the message must still be caught
        // here, because the smart-intent engine runs long before the button
        // handler and would otherwise DELETE the quote context and read
        // "compare" as a two-car comparison ("tell me the two models to
        // compare") instead of the state-wise prices.
        if (/^(compare|compare states?|compare state wise|state ?wise|state prices?|pan[\s-]?india|breakup|break ?up|price breakup|on[\s-]?road breakup|breakdown)\b/i.test(t)) {
          selectedId = 'BTN_REOFFER_ACTIONS';
          msgText = '';
        }
      }

      // A tap is not something the customer typed. WhatsApp sends the button's
      // TITLE as the message body, so "Yes, Compare" reached the smart-intent
      // engine — which runs before the button handler, did not recognise that
      // wording, cleared the quote context and answered the word "compare" as a
      // request to compare two models. Tapping the button never worked.
      if (/^BTN_(PANIND_YES|PRICE_BREAKUP|NEW_QUOTE|REOFFER_ACTIONS)$/.test(String(selectedId || ''))
          || /^INV_CAT_/.test(String(selectedId || ''))) {
        msgText = '';
      }
    } catch (e) {
...
// ==================================================
// INVENTORY DEAL CARD BUTTONS (Features / Specs / Deal & EMI)
// ==================================================
// An escape hatch: start a fresh car without waiting for the old one to expire.
if (_config.photoIngest && msgText && /^\s*(new car|start over|reset car|fresh car)\s*$/i.test(msgText)
    && typeof _config.photoIngest.resetPending === 'function') {
  _config.photoIngest.resetPending(from);
  await _config.waSendText(from, 'Cleared. Send the photos of the next car whenever you like.');
  return res.sendStatus(200);
}

// Typed answers to the category question, for when the buttons don't render.
if (_config.photoIngest && msgText && typeof _config.photoIngest.chooseCategory === 'function'
    && /^\s*(used|pre-?owned|new|demo)\s*$/i.test(msgText)) {
  const word = msgText.trim().toLowerCase();
  const cat = word.startsWith('new') ? 'new' : word.startsWith('demo') ? 'demo' : 'used';
  try {
    const done = await _config.photoIngest.chooseCategory(from, cat);
    if (done) return res.sendStatus(200);
  } catch (e) {}
}

// The dealer telling us what kind of car this is — the one thing the bot
// should never decide for itself.
if (_config.photoIngest && selectedId && /^INV_CAT_(USED|NEW|DEMO)_/.test(String(selectedId))) {
  const cat = String(selectedId).split('_')[2].toLowerCase();
  try {
    const done = await _config.photoIngest.chooseCategory(from, cat);
    if (done) return res.sendStatus(200);
  } catch (e) {
    if (_config.DEBUG) console.warn('category choice failed', e && e.message ? e.message : e);
  }
}

if (_config.inventory && selectedId && String(selectedId).startsWith('INV_')) {
  try {
    const handled = await _config.inventory.handleButton(from, selectedId);
    if (handled) return res.sendStatus(200);
  } catch (e) {
    if (_config.DEBUG) console.warn('inventory button failed', e && e.message ? e.message : e);
  }
}
// Price-list confirm buttons (admins)
if (_config.priceSync && selectedId && String(selectedId).startsWith('PRICE_')) {
  try {
    const handled = await _config.priceSync.handleButton(from, selectedId);
    if (handled) return res.sendStatus(200);
  } catch (e) {
    if (_config.DEBUG) console.warn('priceSync button failed', e && e.message ? e.message : e);
  }
}
// Admin forwards a customer requirement in plain language → tracked lead
if (_config.leadIntake && _config.inventory && msgText &&
    _config.inventory.isDealAdmin(from) &&
    _config.leadIntake.isLeadMessage(msgText, {
      pendingFlow: !!(_config.photoIngest && _config.photoIngest.hasPending(from))
    })) {
  try {
    const handled = await _config.leadIntake.handleForwardedLead(from, msgText);
    if (handled) return res.sendStatus(200);
  } catch (e) {
    if (_config.DEBUG) console.warn('leadIntake failed', e && e.message ? e.message : e);
  }
}
// Admin command: "PRICES" → list active price overrides
if (_config.priceSync && _config.inventory && msgText && /^\s*prices\s*$/i.test(msgText) && _config.inventory.isDealAdmin(from)) {
  await _config.waSendText(from, _config.priceSync.listOverrides());
  return res.sendStatus(200);
}
// Admin command: "EDIT MC-0004 price 6.2 lakh" → fix one field without Studio
if (_config.inventory && msgText && /^\s*edit\b/i.test(msgText) && _config.inventory.isDealAdmin(from)) {
  const handled = await _config.inventory.editAndReply(from, msgText);
  if (handled) return res.sendStatus(200);
}
// Admin command: "PUBLISH MC-0004" → put a draft listing live on the website
if (_config.inventory && msgText && _config.inventory.isDealAdmin(from)) {
  const pm = /^\s*publish\s+(MC-\d+)\s*$/i.exec(msgText);
  if (pm) {
    await _config.inventory.publishAndReply(from, pm[1].toUpperCase());
    return res.sendStatus(200);
  }
}
// Admin command: "BLAST MC-0001 [20]" → send the deal template to matched leads
if (_config.dealBlast && _config.inventory && msgText && _config.inventory.isDealAdmin(from)) {
  const bm = /^\s*blast\s+(MC-\d+)(?:\s+(\d+))?\s*$/i.exec(msgText);
  if (bm) {
    try {
      const out = await _config.dealBlast.blastDeal(bm[1].toUpperCase(), { limit: Number(bm[2]) || 50 });
      await _config.waSendText(from, out.ok
        ? `📣 Blast for *${bm[1].toUpperCase()}*: matched ${out.matched}, sent ${out.sent}, failed ${out.failed}${out.errors ? `\n_${out.errors[0]}_` : ''}`
        : `⚠️ Blast failed: ${out.error}`);
    } catch (e) {
      await _config.waSendText(from, `⚠️ Blast error: ${e.message}`);
    }
    return res.sendStatus(200);
  }
}
...
if (_config.groupIngest) {
  ...
  // Deal-admin price-sheet forwards (image captioned new/demo) skip auto-ingest
  const isDealForward = type === 'image' && /^\s*(new|demo)\b/i.test(msgText || '') &&
    _config.inventory && _config.inventory.isDealAdmin(from);
  // A dealer answering the intake questionnaire types exactly what this detector
  // reads as a listing ("Swift 2019, 45000 km, 6.5 lakh, 1st owner"), so it used
  // to swallow the confirmation and the intake never produced a listing. An open
  // photo intake always owns the next message.
  const awaitingIntakeReply = !!(_config.photoIngest && _config.photoIngest.hasPending(from));
  if (!isDealForward && !awaitingIntakeReply) {
...
// ==================================================
// PHOTO INGEST: dealer sends photo → GPT analyzes → asks for details
// ==================================================
if (_config.photoIngest) {
  // Deal-admin colour text arriving while the photo batch is still collecting
  if (msgText && typeof _config.photoIngest.captureDealText === 'function' &&
      _config.photoIngest.captureDealText(from, msgText)) {
    return res.sendStatus(200);
  }

  // Check if dealer has a pending photo awaiting details
  if (_config.photoIngest.hasPending(from) && msgText) {
    const handled = await _config.photoIngest.handleReply(from, msgText);
    if (handled) return res.sendStatus(200);
  }

  // New photo with no text (or just a short caption) → analyze.
  // Deal-admin captions starting with new/demo always route through (price-sheet forwards).
  if (type === 'image' && msg?.image?.id) {
    const caption = msg.image.caption || '';
    const isDealCaption = /^\s*(new|demo)\b/i.test(caption) &&
      _config.inventory && _config.inventory.isDealAdmin(from);
    const isJustPhoto = !caption || caption.length < 15;
    if (isJustPhoto || isDealCaption) {
      const handled = await _config.photoIngest.handlePhoto(from, name, msg.image.id, caption);
      if (handled) return res.sendStatus(200);
    }
  }
}
...
// ==================================================
// FOLLOW-UP: STOP handler + mark active on any reply
// ==================================================
if (_config.followUp && msgText) {
  // Any reply = customer is active, push follow-up back
  _config.followUp.markActive(from);

  // Handle STOP request
  if (_config.followUp.isStopRequest(msgText)) {
    _config.followUp.stopFollowUp(from);
    await _config.waSendText(from, 'You have been unsubscribed from follow-up messages. You can still use our services anytime.');
    return res.sendStatus(200);
  }
}

// ==================================================
// TRADE-IN: Detect trade-in/exchange requests
// ==================================================
if (_config.tradeIn && msgText) {
  const lastSvcTrade = _config.getLastService(from);

  // If already in TRADE_IN context, treat any car details as trade-in input
  if (lastSvcTrade === 'TRADE_IN') {
    const carDetails = _config.tradeIn.parseCarDetails(msgText);
    if (carDetails) {
      const offer = _config.tradeIn.formatTradeInOffer(carDetails, null, 0);
      await _config.waSendText(from, offer);
      return res.sendStatus(200);
    }
  }

  // Fresh trade-in request (has "trade" or "exchange" keyword)
  const tradeReq = _config.tradeIn.parseTradeInRequest(msgText);
  if (tradeReq) {
    const offer = _config.tradeIn.formatTradeInOffer(tradeReq, null, 0);
    await _config.waSendText(from, offer);
    _config.setLastService(from, 'TRADE_IN');
    return res.sendStatus(200);
  }
}
...
if (numMatch) {
  ...
  if (rec) {
    ...
    const stMatch = rec.stateMatch || 'DELHI';

    // --- Render quote directly from stored row data (no re-search) ---
    if (chosen.row && chosen.brand && chosen.onroad) {
      const mdl = (typeof chosen.idxModel === 'number' && chosen.row[chosen.idxModel]) ? String(chosen.row[chosen.idxModel]).toUpperCase() : '';
      const varr = (typeof chosen.idxVariant === 'number' && chosen.row[chosen.idxVariant]) ? String(chosen.row[chosen.idxVariant]).toUpperCase() : '';
      const fuelStr = chosen.fuel ? String(chosen.fuel).toUpperCase() : '';
      const exShow = chosen.exShow || 0;
      const onroad = chosen.onroad || 0;
      const loanAmt = exShow || onroad;
      const roi = _config.getNewCarROI ? _config.getNewCarROI(loanAmt) : 8.1;
      const emi60 = loanAmt ? _config.calcEmiSimple(loanAmt, roi, 60) : 0;

      const lines = [];
      lines.push(`*${chosen.brand}* ${mdl} ${varr}`);
      lines.push(`*Location:* ${stMatch.toUpperCase()} • *Profile:* INDIVIDUAL`);
      if (fuelStr) lines.push(`*Fuel:* ${fuelStr}`);
      if (exShow) lines.push(`*Ex-Showroom:* ₹ ${_config.fmtMoney(exShow)}`);
      if (onroad) lines.push(`*On-Road (INDIVIDUAL):* ₹ ${_config.fmtMoney(onroad)}`);

      if (loanAmt > 0 && emi60 > 0) {
        lines.push('*🔹 Loan & EMI Options*');
        lines.push('');
        lines.push('*OPTION 1 – NORMAL EMI*');
        lines.push(`Loan Amount: 100% of Ex-Showroom → ₹ ${_config.fmtMoney(loanAmt)}`);
        lines.push(`Tenure: 60 months @ ${roi}% p.a.`);
        lines.push(`Approx EMI: ₹ *${_config.fmtMoney(emi60)}*`);

        // Bullet EMI
        try {
          const bulletSim = _config.simulateBulletPlan ? _config.simulateBulletPlan({ amount: loanAmt, rate: roi, months: 60, bulletPct: 0.25 }) : null;
          if (bulletSim && bulletSim.monthly_emi) {
            const bulletAmt = Math.round(loanAmt * 0.25);
            const perBullet = Math.round(bulletAmt / 5);
            lines.push('');
            lines.push('*OPTION 2 – BULLET EMI (25%)*');
            lines.push(`Monthly EMI (approx): ₹ *${_config.fmtMoney(bulletSim.monthly_emi)}*`);
            lines.push(`Bullet total (25% of loan): ₹ *${_config.fmtMoney(bulletAmt)}*`);
            lines.push('');
            lines.push('*Bullets:*');
            lines.push([12,24,36,48,60].map(m => `₹ ${_config.fmtMoney(perBullet)} at month ${m}`).join('\n'));
          }
        } catch(e) {}
      ...
      // Store context for breakup/compare buttons
      if (!global.panIndiaPrompt) global.panIndiaPrompt = new Map();
      if (typeof _config.noteLeadInterest === 'function') {
        _config.noteLeadInterest(from, `Quoted: ${chosen.brand} ${mdl} ${varr}`);
      }
      global.panIndiaPrompt.set(from, {
        row: chosen.row,
        header: chosen.header || [],
        title: `${chosen.brand} ${mdl} ${varr}`,
        exShow: exShow,
        onroad: onroad,
        stateMatch: stMatch
      });

      console.log('SERIAL_SELECTION: direct render', { brand: chosen.brand, model: mdl, variant: varr, stateMatch: stMatch, onroad });
      await _config.waSendText(from, lines.join('\n'));
      await _config.sendNewCarButtons(from);
      _config.setLastService(from, 'PAN_INDIA_PROMPT');
      _config.incrementQuoteUsage(from);
      return res.sendStatus(200);
    }

    // Fallback: re-query (shouldn't normally reach here)
    let queryText = chosen.title || '';
    if (rec.stateMatch) queryText += ' ' + rec.stateMatch;
    if (queryText) {
  ...
  if (searchRec && Array.isArray(searchRec) && searchRec.length > 0) {
    ...
    if (searchRec[sIdx] && _config.carSearch) {
      ...
      // Calculate EMI for display (slab-based used car ROI)
      let emiInfo = null;
      if (car.price > 0) {
        const loanAmt = Math.round(car.price * 0.85); // 85% LTV
        const usedRates = _config.getUsedCarROI ? _config.getUsedCarROI(car.price) : { visible: 9.99 };
        const rate = usedRates.visible;
        const months = 60;
      ...
      // Dealer group cars have no photo/price — request from dealer via staging
      const isDealerGroupCar = car.source === 'dealer_group' && (!car.images || !car.images.length);

      if (isDealerGroupCar) {
        // Tell customer we're getting details
        await _config.waSendText(from,
          `*${car.title || car.year + ' ' + car.brand + ' ' + car.model}*\n\n` +
          (car.km ? `KM: ${(car.km/1000).toFixed(0)}K | ` : '') +
          (car.fuel ? `${car.fuel} | ` : '') +
          (car.color ? `${car.color} | ` : '') +
          (car.owners || '') + `\n\n` +
          `We're getting the *latest photos and best price* from the dealer for you.\n` +
          `You'll receive the complete details shortly.`
        );

        // Forward to staging to ask dealer
        if (_config.STAGING_API_URL) {
          try {
            const enquiryData = {
              ...
              dealerPhone: car.dealerPhone || car.phone || null,
              dealerName: car.dealerName || car.senderName || null,
              rawText: car.rawText || '',
              requestType: 'PHOTO_PRICE'
            };
            await _config.fetch(`${_config.STAGING_API_URL}/api/enquiry/receive`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(enquiryData)
            });
          } catch (e) {
            console.warn('Dealer photo/price request failed:', e.message);
          }
        }
      ...
      // Normal car (from CarWale/CarDekho) — has photo + price
      const msg = _config.carSearch.formatSingleCar(car, emiInfo);

      // Send car photo first (if available), then details
      if (car.images && car.images.length > 0) {
        const caption = `${car.title || car.year + ' ' + car.brand + ' ' + car.model}`;
        await _config.waSendImageLink(from, car.images[0], caption);
      }
...
if (
  ...
  let rate;
  if (svc === 'LOAN_USED') {
    const usedRates = _config.getUsedCarROI ? _config.getUsedCarROI(amt) : { visible: 9.99, internal: 10.0 };
    rate = usedRates.internal;
  } else {
    rate = _config.getNewCarROI ? _config.getNewCarROI(amt) : _config.NEW_CAR_ROI;
  }
...
// ================= END LOAN EMI HANDLER =================
  ...
  // Staging mode: skip for non-testers
// ------------------------------------------------------------------
if (_isAllowedOnStaging) {
try {
  ...
  console.warn("Smart intent engine failed:", e?.message || e);
...
} // ✅ CLOSES: if (_isAllowedOnStaging) — smart intent guard
    ...
    if (selectedId) {
      ...
      // ============ TYPED "COMPARE"/"BREAKUP" → BRING THE BUTTONS BACK ============
      if (selectedId === 'BTN_REOFFER_ACTIONS') {
        const ctx = global.panIndiaPrompt && global.panIndiaPrompt.get(from);
        if (!ctx) {
          await _config.waSendText(from, 'Ask for a variant price first, then the comparison options appear below it.');
          _config.setLastService(from, 'NEW');
          return res.sendStatus(200);
        }
        await _config.waSendText(from, `*${ctx.title}* — tap below 👇`);
        await _config.sendNewCarButtons(from);
        _config.setLastService(from, 'PAN_INDIA_PROMPT');
        return res.sendStatus(200);
      }

      // ================= PAN-INDIA COMPARE BUTTON =================
      if (selectedId === 'BTN_PANIND_YES') {
        const ctx = global.panIndiaPrompt && global.panIndiaPrompt.get(from);
        if (!ctx || !ctx.row || !ctx.header) {
          await _config.waSendText(from, 'Sorry, I could not retrieve the variant again. Please ask for the quote once more.');
          _config.setLastService(from, 'NEW');
          return res.sendStatus(200);
        }
        const aggregate = _config.extractPanIndiaPricesFromRow(ctx.row, ctx.header);
        const states = Object.keys(aggregate || {});
        if (!states.length) {
          await _config.waSendText(from, 'State-wise pricing is not available for this variant.');
        ...
        out.push('');
        out.push(`🔽 *Lowest:* ${states[0]} — ₹ ${_config.fmtMoney(aggregate[states[0]])}`);
        out.push(`🔼 *Highest:* ${states[states.length - 1]} — ₹ ${_config.fmtMoney(aggregate[states[states.length - 1]])}`);
        out.push('');
        out.push('*State-wise prices:*');
        states.forEach(st => { out.push(`• *${st}* → ₹ ${_config.fmtMoney(aggregate[st])}`); });
        await _config.waSendText(from, out.join('\n'));
        // Context used to be dropped here, so asking for the breakup right after
        // a comparison answered "I could not retrieve the variant". Keep it and
        // put the buttons back, so the next step is a tap too.
        _config.setLastService(from, 'PAN_INDIA_PROMPT');
        await _config.sendNewCarButtons(from);
        return res.sendStatus(200);
      }
      ...
      if (selectedId === 'BTN_PRICE_BREAKUP') {
        const ctx = global.panIndiaPrompt && global.panIndiaPrompt.get(from);
        console.log('BREAKUP_CTX:', ctx ? { title: ctx.title, stateMatch: ctx.stateMatch, exShow: ctx.exShow, onroad: ctx.onroad, hasRow: !!ctx.row, headerLen: (ctx.header||[]).length } : 'NO_CTX');
        if (!ctx) {
          await _config.waSendText(from, 'Sorry, I could not retrieve the variant details. Please ask for the quote once more.');
          _config.setLastService(from, 'NEW');
          return res.sendStatus(200);
        }
        let breakup = null;
        if (ctx.row && ctx.header && ctx.header.length) {
          breakup = _config.extractBreakupFromCSV(ctx.row, ctx.header, ctx.stateMatch);
          console.log('BREAKUP_CSV_RESULT:', breakup ? 'OK' : 'NULL');
        }
        if (!breakup) {
          breakup = _config.calculatePriceBreakup(ctx.exShow, ctx.onroad, ctx.stateMatch);
          console.log('BREAKUP_CALC_RESULT:', breakup ? 'OK' : 'NULL');
        }
        console.log('BREAKUP_RESULT:', breakup ? { hasExact: breakup.hasExactData, exShow: breakup.exShowroom, roadTax: breakup.roadTax, ins: breakup.insuranceAll, total: breakup.total } : null);
        if (!breakup) {
          await _config.waSendText(from, 'Price breakup is not available for this variant.');
          _config.setLastService(from, 'NEW');
          return res.sendStatus(200);
        }
        const text = _config.formatPriceBreakup(breakup, ctx.title, ctx.stateMatch);
        await _config.waSendText(from, text);
        _config.setLastService(from, 'NEW');
        return res.sendStatus(200);
      }
...
case 'BTN_USED_MORE':
  ...
  'Tell us what you want — the more detail, the better results:\n\n' +
  '*Model:*  `Creta` or `BMW X1`\n' +
  '*Budget:*  `SUV around 15 lakh`\n' +
  '*Fuel:*  `Creta diesel` or `petrol` or `hybrid` or `EV`\n' +
  '*Age:*  `Creta under 3 years` or `under 5 years`\n' +
  '*City:*  `Fortuner Mumbai`\n' +
  '*Pan India:*  `GLC Pan India`\n\n' +
  'Examples:\n' +
  '• `Creta diesel under 3 years`\n' +
  '• `Fortuner under 5 years Delhi`\n' +
  '• `BMW X1 petrol Pan India`\n\n' +
  '_Default: Delhi NCR, all fuels, all ages._'
);
return res.sendStatus(200);
        case 'SRV_SELL_CAR':
        ...
        case 'SRV_INSURANCE':
          if (_config.insurance) {
            await _config.insurance.startInsurance(from);
          } else {
            await _config.waSendText(from, 'Insurance service coming soon. Contact us for assistance.');
          }
          return res.sendStatus(200);
        ...
        case 'INS_DECLINE':
          if (_config.insurance) {
            const handled = await _config.insurance.handleMessage(from, msgText, selectedId);
            if (handled) return res.sendStatus(200);
            const qHandled = await _config.insurance.handleQuoteResponse(from, selectedId);
            if (qHandled) return res.sendStatus(200);
          }
          return res.sendStatus(200);
        ...
        case 'SRV_TRADE_IN':
          ...
          await _config.waSendText(
            from,
            '*Trade-In / Exchange*\n\n' +
            'Get your old car valued against a new purchase.\n\n' +
            'Tell us about your car:\n' +
            '`trade in Creta 2020 40000 km diesel 1st owner`\n\n' +
            '_We\'ll give you an estimated value + show your net cost for the new car._'
          );
          return res.sendStatus(200);
...
case 'BTN_LOAN_CUSTOM':
  ...
  return res.sendStatus(200);
    ...
    // Insurance flow — handle active insurance conversation
    if (_config.insurance && _config.insurance.hasActiveState(from)) {
      const handled = await _config.insurance.handleMessage(from, msgText, null);
      if (handled) return res.sendStatus(200);
    }

    // Insurance intent detection (free text like "insurance", "policy renewal")
    if (_config.insurance && msgText && _config.insurance.isInsuranceIntent(msgText)) {
      const lastSvc = _config.getLastService(from);
      if (!lastSvc || !lastSvc.startsWith('INS_')) {
        await _config.insurance.startInsurance(from);
        return res.sendStatus(200);
      }
    }
    ...
    // Greeting — show menu (with or without welcome text)
    {
      ...
      if (looksLikeGreeting) {
        // Staging mode: only admin + testers get full menu
        if (!_isAllowedOnStaging) {
          await _config.waSendText(from, 'Thank you for contacting MR. CAR. Please send car photos or details and we will assist you.');
          return res.sendStatus(200);
        }
  ...
  bulletPct: 0.25                   // ✔ 25%
    ...
    // USED CAR detection (staging: testers only)
    if (_isAllowedOnStaging && type === 'text' && msgText) {
      const textLower = msgText.toLowerCase();
      const explicitNew = /\b(new\s+car|brand\s+new|showroom|on[- ]?road|ex[- ]?showroom|latest\s+price|new\s+model)\b/.test(textLower);
      ...
      // If user explicitly says "new car", skip used car flow entirely
      if (!explicitNew && (explicitUsed || hasYear || lastSvc === 'USED')) {
          ...
          try {
            ...
            if (parsed.model) {
              ...
              if (results.length > 0) {
                ...
                // Track for follow-up + price alerts
                if (_config.followUp) {
                  _config.followUp.addToQueue({ phone: from, name, model: parsed.model, lastQuotePrice: results[0]?.price || 0 });
                }
                if (_config.priceAlerts && results[0]) {
                  _config.priceAlerts.trackPrice({ phone: from, name, model: parsed.model, city: searchCity, quotedPrice: results[0].price });
                }
    ...
    // BUDGET WITH NO MODEL — "under 15 lakh", "budget 25 lakh".
    // The quote engine's variant list needs a model to work from, so a bare
    // budget fell through and the bot said nothing. This is the same engine the
    // website answers with, so the two cannot drift apart. A price typed while a
    // dealer's photo batch is open never reaches here — that is captured much
    // earlier and kept with the car.
    if (type === 'text' && msgText && _config.variantSearch) {
      try {
        const budget = _config.variantSearch.parseBudget(msgText);
        const modelPart = _config.variantSearch.stripBudgetWords(msgText).replace(/[^a-z0-9]/gi, '');
        if (budget) {
          // A leftover word that names no car — "best suv under 20 lakh" — would
          // otherwise match nothing and answer with silence. If the worded search
          // finds nothing, fall back to the budget alone rather than give up.
          let out = modelPart.length < 3
            ? await _config.variantSearch.search({ query: msgText, city: msgText })
            : await _config.variantSearch.search({ query: msgText, city: msgText });
          let cars = (out && out.cars) || [];
          if (!cars.length && modelPart.length >= 3) {
            out = await _config.variantSearch.search({ query: `under ${budget}`, city: msgText });
            cars = (out && out.cars) || [];
          }
          if (cars.length) {
            const L = n => `₹ ${Number(n).toLocaleString('en-IN')}`;
            const lakh = v => (v >= 1e7
              ? `${(v / 1e7).toFixed(2).replace(/\.?0+$/, '')} Cr`
              : `${(v / 1e5).toFixed(2).replace(/\.?0+$/, '')} Lakh`);
            const lines = [];
            lines.push(`*${cars.length} cars within ₹${lakh(budget)}* — on-road in ${out.state}`);
            lines.push('');
            cars.slice(0, 10).forEach((c, i) => {
              const name = [c.brand, c.model, c.variant].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim();
              lines.push(`${i + 1}) *${name}*`);
              lines.push(`    ${L(c.price)} on-road${c.emi ? `  ·  EMI ${L(c.emi)}/mo` : ''}`);
            });
            lines.push('');
            lines.push('_Reply with a model name for the full variant list and price breakup._');
            await _config.waSendText(from, lines.join('\n'));
            _config.setLastService(from, 'NEW');
            return res.sendStatus(200);
          }
        ...
        console.warn('budget search failed:', e && e.message ? e.message : e);
      }
    }
