...
if (numMatch) {
  ...
  if (rec) {
    ...
    if (queryText) {
      // Preserve state context (e.g. UP, HR) from original query
      if (rec.stateMatch) {
        queryText += ' ' + rec.stateMatch;
      }
      console.log('SERIAL_SELECTION: queryText=', queryText, 'stateMatch=', rec.stateMatch || 'NONE');
