...
async function search(query = {}) {
  ...
  // Filter by city — only show cars from the same city/region (skip if pan india)
  if (city && !query.panIndia) {
    const cityRegion = CITY_REGIONS[city] || [city];
    all = all.filter(c => {
      const carCity = (c.city || '').toLowerCase();
      if (!carCity) return true; // if no city data, include it (benefit of doubt)
...
function formatSingleCar(car, emiInfo = null) {
  ...
  msg += `_MR. CAR handles everything — no hassle for you._`;
