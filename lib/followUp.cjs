...
// Same reasoning as the lead store: a deploy must not wipe pending follow-ups
const FOLLOWUP_DIR = process.env.INVENTORY_DATA_DIR || path.join(__dirname, '..');
const FOLLOWUP_FILE = path.join(FOLLOWUP_DIR, 'followup_queue.json');
...
async function _processQueue() {
  ...
  for (const lead of queue) {
    ...
    try {
      const r = await _config.waSendText(lead.phone, msg);
      // Plain text only reaches someone who has messaged us in the last 24
      // hours. A lead we were handed — from the website, or forwarded by the
      // desk — has never opened that window, so the nudge silently failed and
      // the queue counted it as sent. Open the conversation with an approved
      // template instead; once they reply, the rest of the sequence goes as
      // normal text.
      const delivered = !!(r && (r.messages || (r.resp && r.resp.messages) || r.ok));
      if (!delivered && !lead.openedWithTemplate && typeof _config.waSendTemplate === 'function') {
        const first = String(lead.name || '').trim().split(/\s+/)[0] || 'there';
        await _config.waSendTemplate(lead.phone, 'mr_car_broadcast_en', [
          { type: 'body', parameters: [{ type: 'text', text: first }] }
        ]);
        lead.openedWithTemplate = true;
        console.log(`FollowUp: no open window for ${lead.phone} — opened with a template`);
      } else {
