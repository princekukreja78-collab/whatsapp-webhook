...
// Rebuild the shareable reel on demand (Studio 🎬 button). Synchronous so the
// admin sees the real result — a render is ~10-15s for six photos.
router.post('/api/inventory/cars/:id/video', async (req, res) => {
  try {
    if (!_config.dealVideo) return res.status(503).json({ ok: false, error: 'video engine not configured' });
    const car = findCar(req.params.id);
    if (!car) return res.status(404).json({ ok: false, error: 'Not found' });
    const r = await _config.dealVideo.build(car, { force: true });
