...
function createFromIngest({ entry, photos, rc }) {
  ...
  _save();
  // Fire-and-forget AI fill of specs/features
...
async function sendCarCard(to, carId) {
  ...
  const caption = buildCardCaption(car);
  const photoUrl = _publicPhotoUrl(car);

  let cardRes = null;
  if (photoUrl && typeof _config.waSendImageLink === 'function') {
    cardRes = await _config.waSendImageLink(to, photoUrl, caption);
  } else if (typeof _config.waSendText === 'function') {
    cardRes = await _config.waSendText(to, caption);
  }
  const btnRes = await _sendExploreButtons(to, car, 'NONE');

  const cardErr = _waError(cardRes);
  const btnErr = _waError(btnRes);
  const delivered =
    !!(cardRes && (cardRes.messages || (cardRes.ok && cardRes.resp?.messages))) ||
    !!(btnRes && btnRes.messages);

  if (!delivered) {
    return { ok: false, error: cardErr || btnErr || 'Message not accepted by WhatsApp' };
  }
  return { ok: true, warning: cardErr || btnErr || undefined };
}
