...
/**
 * Balloon / residual-value plan (Bajaj FlexDrive+ style): part of the loan is parked
 * as a balloon (the car's estimated resale value) payable at the end of the tenure,
 * so the monthly EMI drops. If the balloon isn't paid it converts into an extension
 * loan over `extensionTenure` months.
 *   EMI = termEMI − B·r / ((1+r)^n − 1)
 */
function balloonPlan(financed, roi, months, scheme) {
  const r = (Number(roi) || 0) / 1200;
  const n = Number(months) || 0;
  const pct = Number(scheme.balloonPct) || 35;
  const B = Math.round(financed * (pct / 100));
  if (!financed || !n || !B) return null;
  const f = Math.pow(1 + r, n);
  const termEmi = calcEmi(financed, roi, n);
  const emi = r > 0 ? Math.round(termEmi - (B * r) / (f - 1)) : Math.round((financed - B) / n);
...
function emiRows(price, scheme) {
  ...
  return tenures.map(t => {
    ...
    const row = {
      ...
      bullet: null,
      balloon: plans.includes('balloon') ? balloonPlan(financed, roi, t, scheme) : null
    };
...
router.put('/api/inventory/loan-schemes/:id', (req, res) => {
  ...
  const b = req.body || {};
  for (const k of ['bank', 'roi', 'maxTenure', 'minDownPct', 'processingFee', 'notes', 'bulletPct', 'balloonPct', 'extensionTenure']) {
...
// Website search — matches LIVE inventory (make/model/colour/type)
router.post('/api/vehyra/search', (req, res) => {
  const q = String(req.body?.query || req.body?.msgText || '').toLowerCase().trim();
  if (!q) return res.json({ ok: false, error: 'Query missing' });
  ...
  const cars = db.cars
    ...
    .map(c => {
      ...
      const s = _schemesForCar(c)[0];
      const financed = Math.round(price * (1 - (Number(s.minDownPct) || 10) / 100));
      const cover = (c.photos || []).find(p => p.cover) || (c.photos || [])[0];
      return {
        id: c.id,
        model: carTitle(c),
        variant: [c.colour, c.km ? `${Number(c.km).toLocaleString('en-IN')} km` : null, c.fuel].filter(Boolean).join(' • ') || (c.type === 'new' ? 'New car' : 'Pre-owned'),
        price,
        emi: calcEmi(financed, s.roi, 60),
        tags: [c.type === 'new' ? 'New' : 'Pre-Owned', 'In Stock'],
        cover: cover ? cover.url : null,
        url: `/cars/${c.id}`
      };
    });
  res.json({ ok: true, cars, source: 'inventory' });
});
