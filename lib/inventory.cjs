...
function init(config) {
  ...
  // Only ever conjure the directories for a local checkout. When a persistent
  // disk is configured, creating the path ourselves is the worst thing we can
  // do: the disk isn't mounted, we make an empty folder in its place, and the
  // store looks like a dealership with no cars — so the next listing takes
  // MC-0001 and is lost when the real disk comes back.
  const onConfiguredDisk = !!process.env.INVENTORY_DATA_DIR;
  if (!onConfiguredDisk) {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
    if (!fs.existsSync(MEDIA_ROOT)) fs.mkdirSync(MEDIA_ROOT, { recursive: true });
  } else if (!fs.existsSync(DATA_DIR)) {
    console.warn('Inventory: INVENTORY_DATA_DIR does not exist — the disk is not mounted; listings are refused until it is');
  }
  _load();
  // A restart is not a reason to lose an upload. Render attaches the disk a
  // moment after the process starts, so keep looking for it rather than
  // spending the whole deploy refusing listings.
  if (!storeReady()) {
    let tries = 0;
    const t = setInterval(() => {
      tries += 1;
      _load();
      if (storeReady()) {
        console.log(`Inventory: disk appeared after ${tries * 2}s — ${db.cars.length} cars`);
        clearInterval(t);
      } else if (tries >= 30) {
        console.error('Inventory: disk still not mounted after 60s — listings stay refused');
        clearInterval(t);
      }
    }, 2000);
    t.unref?.();
...
function isDealAdmin(from) {
  ...
  allowed.push('919810212743'); // authorised for photo + detail uploads
...
// Have we actually got the real store in hand? A persistent disk that is not
// mounted yet looks exactly like "no cars ever": db stays empty, seq stays 1,
// and the next car created is handed MC-0001 — an id an existing car already
// owns. It is written to nothing, the next reload wipes it, and the dealer is
// told their car is live.
let _storeReady = false;
function storeReady() { return _storeReady && storeHealthy(); }
  ...
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      if (raw && Array.isArray(raw.cars)) {
        db = { seq: 1, schemeSeq: 1, loanSchemes: [], ...raw };
        _storeReady = true;
      }
      _lastMtime = fs.statSync(DATA_FILE).mtimeMs;
    } else if (storeHealthy()) {
      // First boot on a genuinely empty disk: claim it by writing the empty
      // store now. After this, a MISSING file means the disk is absent — not
      // that there are no cars — and creates are refused instead of vanishing.
      _save();
      _storeReady = true;
      console.log('Inventory: empty store initialised at', DATA_FILE);
    } else {
      console.warn('Inventory: data dir not writable — refusing to create listings until it is');
    }
 ...
 */
// A Starter disk is 1GB, and photos and reels share it. Report what is left so
// it is noticed before a dealer's upload is the thing that fills it.
function _diskUsage() {
  try {
    ...
    const walk = dir => {
      for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (e.isDirectory()) walk(f); else media += fs.statSync(f).size;
      }
    };
    try { if (fs.existsSync(MEDIA_ROOT)) walk(MEDIA_ROOT); } catch (e) {}
    return {
      totalMB: mb(total), freeMB: mb(free), usedPct: total ? Math.round(((total - free) / total) * 100) : null,
      photosMB: mb(media)
    };
...
// ---------------- helpers ----------------
function _nextCarId() {
  // The counter is the intent, but the cars on disk are the truth. Start from
  // whichever is higher, so a store that loses seq still numbers the next car
  // after the last one rather than reissuing MC-0001, and skip anything taken.
  const highest = db.cars.reduce((max, c) => {
    const n = Number(String(c.id || '').replace(/^MC-/i, '')) || 0;
    return n > max ? n : max;
  }, 0);
  if (db.seq <= highest) db.seq = highest + 1;
  let id;
  do {
    id = `MC-${String(db.seq).padStart(4, '0')}`;
    db.seq += 1;
  } while (db.cars.some(c => c.id === id));
  return id;
}
...
function _schemesForCar(car) {
  ...
  // Nothing attached: use the schemes marked as the default for this kind of
  // car, so the rate a listing quotes is the one set in Studio rather than a
  // number compiled into the code.
  const kind = car.type === 'new' ? 'new' : 'used';
  const defaults = db.loanSchemes.filter(s => {
    const d = String(s.defaultFor || '').toLowerCase();
    return d === kind || d === 'both';
  });
  if (defaults.length) return defaults;

  // No scheme attached yet — still show all three ways to fund the car, so a
  // freshly added listing is never quoted with fewer options than the rest.
  const isNew = car.type === 'new';
  const roi = isNew ? 8.75 : 9.5;
  return [
    {
      id: 'DEFAULT', bank: 'Mr. Car Finance Partner', roi,
      maxTenure: 72, minDownPct: 10, processingFee: 0,
      // The comment above promised three ways to fund the car and the list gave
      // two, so every listing showed a dash under Flex Pay.
      plans: ['reducing', 'bullet', 'balloon'], bulletPct: 25,
      balloonPct: 35, extensionTenure: 36
    },
    {
      id: 'DEFAULT-FLEX', bank: 'Bajaj Finance SmartDrive+', roi: isNew ? 12 : 13,
      maxTenure: 60, minDownPct: isNew ? 10 : 15, processingFee: 0,
      plans: ['reducing', 'balloon'],
      balloonPreset: isNew ? 'new' : 'used',
      balloonCategory: isNew ? 'High Selling OEMs' : '60-83 months',
      extensionTenure: 36, surakshaAmount: 10000
    }
  ];
}
 ...
 */
function createFromIngest({ entry, photos, rc, documentIndexes }) {
  ...
  // The dealer chose this on the way in; a demo car is a new car that has run,
  // so it lists as new and says so in the notes.
  const category = String(entry.category || 'used').toLowerCase();
  const car = createCar({
    type: category === 'used' ? 'used' : 'new',
    ...
    sourceDealerName: entry.dealerName || '',
    sourceDealerPhone: entry.dealerPhone || '',
    sourceDealerCity: entry.regCity || '',
    consignment: !!entry.dealerPhone,
    price: { asking: parseLakh(entry.askingPrice), offer: parseLakh(entry.lastPrice) },
    notes: [
      category === 'demo' ? 'Demo car — registered, dealership-run' : '',
  ...
  const docSet = new Set((documentIndexes || []).map(Number));
  (photos || []).forEach((p, i) => {
    const ph = _savePhotoFromDataUri(car.id, p, i);
    if (!ph) return;
    if (docSet.has(i)) ph.private = true; // RC card / paperwork — kept, never shown
    car.photos.push(ph);
  });
  const firstPublic = publicPhotos(car)[0];
  if (firstPublic && !publicPhotos(car).some(x => x.cover)) firstPublic.cover = true;
  _save();
...
const CAR_FIELDS = [
  ...
  // Whose car this is. Consignment stock is listed for a partner dealer — we carry
  // no money in it, so these stay internal and never reach the public API.
  'sourceDealerName', 'sourceDealerPhone', 'sourceDealerCity', 'consignment', 'tradePrice'
];
...
function createCar(body) {
  _reloadIfStale();
  if (!storeReady()) {
    // Better a dealer resends photos than a listing that exists for ninety
    // seconds and takes an id belonging to somebody else's car.
    throw new Error('storage is not mounted right now (deploy in progress) — please try again in a minute');
  }
  const car = {
    id: _nextCarId(),
    status: 'draft',
...
function addPhotos(id, files) {
  const car = findCar(id);
  // multer has already written them next to the car's other photos, so an
  // unknown id leaves orphans behind unless we clear them.
  if (!car) {
  ...
  for (const f of files || []) {
    ...
    car.photos.push({
      file: name,
      url: `/media_store/inventory/${id}/${name}`,
      cover: publicPhotos(car).length === 0
    });
    added.push(name);
  }
  car.updatedAt = new Date().toISOString();
...
/**
 * Paperwork — an RC card, an insurance copy, a price sheet — carries the
 * registration number, the owner's name and the chassis number. It is captured
 * on purpose at intake and kept for our own records, but it never belongs on a
 * public listing, in a WhatsApp gallery or in a reel. Everything customer-facing
 * reads photos through here.
 */
function publicPhotos(car) {
  return ((car && car.photos) || []).filter(p => !p.private);
}

function setPhotoPrivate(carId, file, isPrivate) {
  const car = findCar(carId);
  if (!car) return null;
  const photo = (car.photos || []).find(p => p.file === file);
  if (!photo) return null;
  photo.private = !!isPrivate;
  // A hidden photo can't stay the cover, or the listing card goes blank.
  if (photo.private && photo.cover) {
    photo.cover = false;
    const next = publicPhotos(car)[0];
...
function _leanCar(car) {
  const shown = publicPhotos(car);
  const cover = shown.find(p => p.cover) || shown[0] || null;
  return {
...
function _publicPhotoUrl(car) {
  ...
  const shown = publicPhotos(car);
  const cover = shown.find(p => p.cover) || shown[0];
 ...
 */
/**
 * Whenever photos land, refresh both derived assets for a listing: the AI hero
 * pick first, then the shareable reel. Order matters — the reel opens on the
 * cover photo, so the hero has to be settled before it renders. Fire-and-forget:
 * neither asset is worth failing an upload over. Works the same for a new-car
 * deal and a pre-owned listing.
 */
function refreshMedia(id) {
  return selectHeroPhoto(id)
    .catch(e => console.warn('Inventory: hero selection failed', e && e.message))
    .then(() => {
      if (!_config.dealVideo) return null;
      const car = findCar(id);
      if (!car) return null;
      return _config.dealVideo.build(car, { force: true })
        .then(r => {
          if (r && r.ok) console.log('Inventory: reel ready', id, `${r.seconds}s`, `${Math.round(r.bytes / 1024)}KB`);
          else console.warn('Inventory: reel skipped —', r && r.error);
          return r;
        })
        .catch(e => { console.warn('Inventory: reel build failed', e && e.message); return null; });
    });
}

async function selectHeroPhoto(id) {
  const car = findCar(id);
  if (!car || !_config.openai || publicPhotos(car).length < 2) return car;

  // Six is enough to find a front shot, and a 4MB photo base64s to 5.3MB of
  // string — ten of those in one request is how this box runs out of memory.
  const MAX_HERO_BYTES = 3 * 1024 * 1024;
  const candidates = publicPhotos(car).slice(0, 6);
  const images = [];
  // The model numbers what it is SENT, so the photos that make it into the
  // request are tracked separately — skipping one (too big, unreadable) used to
  // shift every number after it and set the cover to the wrong photo.
  const sent = [];
  for (const p of candidates) {
    try {
      const file = path.join(MEDIA_ROOT, id, p.file);
      if (fs.statSync(file).size > MAX_HERO_BYTES) continue;
  ...
  if (n >= 1 && n <= sent.length) {
    car.photos.forEach(p => { p.cover = false; });
    sent[n - 1].cover = true;
    car.updatedAt = new Date().toISOString();
    _save();
    console.log(`Inventory: AI hero for ${id} → photo ${n} (${sent[n - 1].file})`);
  }
  return car;
}
...
// How many photos go into the chat before we hand over to the car's page.
const PICS_ON_WHATSAPP = Number(process.env.PICS_ON_WHATSAPP || 4);

/**
 * Photos are delivered IN the chat, four at a time, because a tap on More Photos
 * sits inside the 24-hour service window and costs nothing. Nobody has to open a
 * link to see the car. Only once the gallery runs out do we offer the car's page
 * — and the URL rides inside the button, never as visible text.
 */
async function _sendPhotoPage(from, car, offset) {
  const base = String(_config.PUBLIC_BASE_URL || process.env.PUBLIC_BASE_URL || '').replace(/\/+$/, '');
  const gallery = publicPhotos(car).filter(p => !p.cover);
  const page = base ? gallery.slice(offset, offset + PICS_ON_WHATSAPP) : [];

  for (const p of page) await _config.waSendImageLink(from, `${base}${p.url}`, '');
  // Let the images land before the buttons so they don't overtake them
  if (page.length) await new Promise(r => setTimeout(r, 3000));

  const nextOffset = offset + page.length;
  const remaining = Math.max(0, gallery.length - nextOffset);

  if (remaining > 0) {
    const interactive = {
      type: 'button',
      body: { text: `${remaining} more photo${remaining > 1 ? 's' : ''} of the *${carTitle(car)}* — see them right here 👇` },
      footer: { text: 'VehYra by MR. CAR' },
      action: {
        buttons: [
          { type: 'reply', reply: { id: `INV_PAGE_${car.id}:${nextOffset}`, title: '🖼 More photos' } },
          { type: 'reply', reply: { id: `INV_DEAL_${car.id}`, title: '💰 Deal & EMI' } },
          { type: 'reply', reply: { id: `INV_CALL_${car.id}`, title: '📞 Talk to Sales' } }
        ]
      }
    };
    return _config.waSendRaw({ messaging_product: 'whatsapp', to: from, type: 'interactive', interactive });
  }
  ...
  if (!page.length && !offset) {
    ...
    return _sendExploreButtons(from, car, 'PICS');
  }

  // Gallery finished. A live listing has a page worth offering; a draft does not.
  if (car.status === 'live') return _sendCarPageButton(from, car, gallery.length + 1);
  return _sendExploreButtons(from, car, 'PICS');
}

/**
 * The car's own page behind a plain button. WhatsApp shows only the label — the
 * URL never appears in the chat, so there is no raw link for anyone to distrust.
 */
async function _sendCarPageButton(to, car, shown) {
  const url = `${SITE_URL()}/cars/${car.id}`;
  const body = `That's all ${shown} photo${shown > 1 ? 's' : ''} of the *${carTitle(car)}*.\n\n` +
    `Full details — specs, features, EMI options and the video walkaround — are on its page.`;
  try {
    return await _config.waSendRaw({
      messaging_product: 'whatsapp', to, type: 'interactive',
      interactive: {
        type: 'cta_url',
        body: { text: body },
        footer: { text: 'VehYra by MR. CAR' },
        action: { name: 'cta_url', parameters: { display_text: '📸 Pics & details', url } }
      }
    });
  } catch (e) {
    // Some numbers/tiers reject cta_url — fall back so the customer isn't stranded.
    console.warn('Inventory: cta_url failed, sending plain link', e.message);
    return _config.waSendText(to, `${body}\n👉 ${url}`);
  }
}
...
async function sendShareableCard(to, car) {
  ...
  const ok = r => !!(r && (r.messages || (r.ok && r.resp?.messages)));
  let sentAnything = false;

  // Both assets go out, in this order and awaited one after the other so they
  // never arrive the wrong way round: the reel earns the watch, the still card
  // is the thing that gets forwarded, saved or printed for a windshield.
  if (_config.dealVideo && _config.dealVideo.existsFor(car.id)) {
    const videoUrl = `${SITE_URL()}/api/vehyra/media/inventory/${car.id}/reel.mp4`;
    const v = await _config.waSendVideoLink(to, videoUrl, caption);
    if (ok(v)) sentAnything = true;
    else console.warn('Inventory: reel send failed —', v && v.error);
  }

  // The card carries a short caption when the reel already said the long version,
  // so the second message reads as a companion rather than a repeat.
  const cardCaption = sentAnything
    ? `📇 *${carTitle(car)}* — save or forward this card.\n📞 MR. CAR • Ashok Vihar, Delhi`
    : caption;
  const r = await _config.waSendImageLink(to, cardUrl, cardCaption);
  if (ok(r)) sentAnything = true;

  if (!sentAnything) {
    // Neither asset rendered — they still get the details
    await _config.waSendText(to, caption);
  }

  // A share is real intent, so it starts the nudge sequence. Never for an admin
  // or the business number itself, and failures here must not break the send.
  try {
    if (_config.followUp && typeof _config.followUp.addToQueue === 'function' && !isDealAdmin(to)) {
      _config.followUp.addToQueue({
        phone: to,
        name: '',
        model: carTitle(car),
        lastQuotePrice: price || 0
      });
    }
  } catch (e) {
    console.warn('Inventory: follow-up queue skipped —', e && e.message);
  }
...
// ---- WhatsApp EDIT: fix one field on a listing without opening Studio ----

// What a dealer types → what the listing calls it.
const EDIT_FIELDS = {
  price: 'asking', asking: 'asking', ask: 'asking', demand: 'asking',
  last: 'offer', offer: 'offer', best: 'offer', final: 'offer', bottom: 'offer',
  km: 'km', kms: 'km', kilometers: 'km', kilometres: 'km',
  year: 'year', make: 'make', brand: 'make', model: 'model', variant: 'variant',
  colour: 'colour', color: 'colour', fuel: 'fuel',
  transmission: 'transmission', gearbox: 'transmission',
  owner: 'owners', owners: 'owners',
  registration: 'registration', reg: 'registration', city: 'registration',
  notes: 'notes', note: 'notes'
};
const EDIT_LABEL = {
  asking: 'Asking price', offer: 'Best offer', km: 'KM driven', year: 'Year',
  make: 'Make', model: 'Model', variant: 'Variant', colour: 'Colour', fuel: 'Fuel',
  transmission: 'Transmission', owners: 'Owners', registration: 'Registration', notes: 'Notes'
};

function _toRupees(v) {
  const s = String(v || '').toLowerCase();
  const n = parseFloat(s.replace(/[^\d.]/g, '')) || 0;
  // "52k" has no word boundary between the digit and the k, so \bk\b never fired
  // and 52k came back as 52.
  if (/lakh|lac/.test(s) || /\d\s*l\b/.test(s)) return Math.round(n * 100000);
  if (/\d\s*k\b/.test(s)) return Math.round(n * 1000);
  return Math.round(n);
}

/**
 * "EDIT MC-0004 price 6.2 lakh". The id is optional — without one it edits the
 * newest listing that came from this dealer's own number, which is nearly always
 * the car they just sent.
 */
function editFromText(from, text) {
  const m = /^\s*edit\s+(?:(MC-\d+)\s+)?([a-z]+)\s+(.+?)\s*$/i.exec(String(text || ''));
  if (!m) return null; // not an edit command — let other handlers have it
  const [, rawId, rawField, rawValue] = m;
  const field = EDIT_FIELDS[rawField.toLowerCase()];
  if (!field) {
    return { ok: false, error: `I don't know the field *${rawField}*. Try: ${[...new Set(Object.values(EDIT_FIELDS))].map(f => EDIT_LABEL[f].toLowerCase()).join(', ')}.` };
  }

  let car = rawId ? findCar(rawId.toUpperCase()) : null;
  if (!car && !rawId) {
    const digits = String(from || '').replace(/[^\d]/g, '');
    car = db.cars
      .filter(c => String(c.sourceDealerPhone || '').replace(/[^\d]/g, '') === digits)
      .sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')))[0] || null;
  }
  if (!car) {
    return { ok: false, error: rawId ? `Listing *${rawId.toUpperCase()}* not found.` : 'Which listing? Send it as *EDIT MC-0004 price 6.2 lakh*.' };
  }

  let shown;
  if (field === 'asking' || field === 'offer') {
    const rupees = _toRupees(rawValue);
    if (!rupees) return { ok: false, error: `*${rawValue}* is not a price I can read. Try *6.2 lakh* or *620000*.` };
    car.price = car.price || {};
    car.price[field] = rupees;
    shown = `₹${rupees.toLocaleString('en-IN')}`;
  } else if (field === 'km') {
    const km = _toRupees(rawValue); // same reader: "45000", "45k", "1.2 lakh"
    if (!km) return { ok: false, error: `*${rawValue}* is not a KM figure I can read.` };
    car.km = km;
    shown = `${km.toLocaleString('en-IN')} km`;
  } else if (field === 'year') {
    const y = Number(String(rawValue).replace(/[^\d]/g, ''));
    if (!(y >= 1990 && y <= new Date().getFullYear() + 1)) return { ok: false, error: `*${rawValue}* is not a year.` };
    car.year = y;
    shown = String(y);
  } else {
    car[field] = String(rawValue).trim();
    shown = car[field];
  }

  car.updatedAt = new Date().toISOString();
  _save();
  console.log(`Inventory: ${car.id} ${field} edited over WhatsApp by ${from}`);
  return { ok: true, car, field, shown };
}

async function editAndReply(from, text) {
  const out = editFromText(from, text);
  if (!out) return false;
  if (!out.ok) { await _config.waSendText(from, `⚠️ ${out.error}`); return true; }
  const price = _effectivePrice(out.car);
  await _config.waSendText(from,
    `✏️ *${out.car.id}* — ${EDIT_LABEL[out.field]} → *${out.shown}*\n\n` +
    `${carTitle(out.car)}\n` +
    `${out.car.km ? Number(out.car.km).toLocaleString('en-IN') + ' km • ' : ''}${price ? '₹' + price.toLocaleString('en-IN') : 'no price yet'}` +
    `${out.car.status === 'live' ? '\n\n_Live on the site — the change is already showing._' : `\n\n_Still a draft. Reply *PUBLISH ${out.car.id}* to put it live._`}`
  );
  return true;
}

/**
 * One-tap publish for a draft that came in over WhatsApp. A listing only goes
 * live with a price and at least one photo — an empty card on the site is worse
 * than no card. Returns { ok, error, car }.
 */
function publishCar(carId) {
  const car = findCar(carId);
  if (!car) return { ok: false, error: 'not_found' };
  if (car.status === 'live') return { ok: false, error: 'already_live', car };
  const missing = [];
  if (!Number(car.price && (car.price.asking || car.price.offer))) missing.push('price');
  if (!publicPhotos(car).length) missing.push('photo');
  if (missing.length) return { ok: false, error: `needs a ${missing.join(' and ')}`, car };
  car.status = 'live';
  car.updatedAt = new Date().toISOString();
  _save();
  console.log(`Inventory: ${car.id} published from WhatsApp`);
  return { ok: true, car };
}

// Offer the publish step where the draft was created, so an intake finishes on
// WhatsApp instead of needing a trip to Inventory Studio.
async function sendPublishPrompt(to, car) {
  const interactive = {
    type: 'button',
    body: {
      text: `📋 *${car.id}* is a DRAFT — it is not on the site yet.\n` +
            `${carTitle(car)}\n\nTap *Publish* to put it live on the website.`
    },
    footer: { text: 'VehYra by MR. CAR' },
    action: {
      buttons: [
        { type: 'reply', reply: { id: `INV_PUB_${car.id}`, title: '🚀 Publish now' } },
        { type: 'reply', reply: { id: `INV_KEEP_${car.id}`, title: '📝 Keep as draft' } }
      ]
    }
  };
  return _config.waSendRaw({ messaging_product: 'whatsapp', to, type: 'interactive', interactive });
...
// Shared by the button and the "PUBLISH MC-0004" text command.
async function publishAndReply(from, carId) {
  const out = publishCar(carId);
  if (out.error === 'not_found') {
    await _config.waSendText(from, `Listing *${carId}* not found.`);
  } else if (out.error === 'already_live') {
    await _config.waSendText(from, `✅ *${out.car.id}* is already live — ${SITE_URL()}/cars/${out.car.id}`);
  } else if (!out.ok) {
    await _config.waSendText(from, `⚠️ *${out.car.id}* ${out.error} before it can go live. Add it in Inventory Studio, then publish again.`);
  } else {
    await _config.waSendText(from,
      `🚀 *${carTitle(out.car)}* is LIVE on the site.\n${SITE_URL()}/cars/${out.car.id}`);
  }
  return true;
}

async function handleButton(from, selectedId) {
  const m = /^INV_(FEAT|SPEC|DEAL|PICS|PAGE|TD|CALL|EXT|EXP|SHARE|PUB|KEEP)_(.+)$/.exec(String(selectedId || ''));
  if (!m) return false;
  const [, action, rawId] = m;
  // "More photos" carries where to continue from in the button id itself, so
  // paging survives a restart — an in-memory cursor would not.
  const pageM = /^(.+):(\d+)$/.exec(rawId);
  const carId = pageM ? pageM[1] : rawId;
  const photoOffset = pageM ? Number(pageM[2]) || 0 : 0;

  // Admin-only: publish / keep-as-draft from the intake confirmation
  if (action === 'PUB' || action === 'KEEP') {
    if (!isDealAdmin(from)) return true; // silently ignore from non-admins
    if (action === 'KEEP') {
      await _config.waSendText(from, `📝 *${carId}* stays a draft — publish it any time with *PUBLISH ${carId}* or in Inventory Studio.`);
      return true;
    }
    return publishAndReply(from, carId);
  }
  ...
  try {
    ...
    } else if (action === 'PICS' || action === 'PAGE') {
      await _sendPhotoPage(from, car, photoOffset);
...
/**
 * Flex Pay (balloon) lines for the WhatsApp quote engines, so a customer sees the
 * same third option on a quote that they see on a deal card or the website.
 * Uses a configured balloon scheme when one exists, else the financier's default table.
 */
function flexPayLines({ amount, roi, months, used }) {
  try {
    const loan = Number(amount) || 0;
    if (!loan) return null;

    let scheme = (db.loanSchemes || []).find(s => schemePlans(s).includes('balloon'));
    if (!scheme) {
      scheme = {
        bank: 'Bajaj Finance SmartDrive+',
        balloonPreset: used ? 'used' : 'new',
        balloonCategory: used ? '60-83 months' : 'High Selling OEMs',
        extensionTenure: 36,
        surakshaAmount: 10000
      };
    }
    const preset = BALLOON_MATRICES[scheme.balloonPreset || (used ? 'used' : 'new')];
    const tenures = preset ? preset.tenures : [36, 48, 60];
    // nearest tenure the financier actually offers
    const n = tenures.reduce((a, b) => Math.abs(b - (months || 48)) < Math.abs(a - (months || 48)) ? b : a, tenures[0]);

    const rate = Number(roi) || (used ? 13 : 12);
    const premium = Number(scheme.surakshaAmount) || 0;
    const plan = balloonPlan(loan, rate, n, scheme, premium);
    if (!plan) return null;

    const money = v => Number(v).toLocaleString('en-IN');
    const lines = [];
    lines.push(`*OPTION 3 – FLEX PAY / BALLOON* (${scheme.bank})`);
    lines.push(`Tenure: ${n} months @ ${rate}% p.a.`);
    if (premium) lines.push(`Loan + Loan Suraksha ₹${money(premium)} → ₹${money(plan.emiBase)}`);
    lines.push(`Monthly EMI: ₹ *${money(plan.emi)}*  _(₹${money(plan.savingPerMonth)} lighter than normal)_`);
    lines.push(`Balloon at end (${plan.balloonPct}%): ₹ *${money(plan.balloonAmount)}*`);
    lines.push(`Pay it, refinance it, or hand the car back.`);
    lines.push(`If not paid: extends ${plan.extensionMonths} months @ ₹${money(plan.extensionEmi)}/mo`);
    return lines;
  } catch (e) {
    console.warn('flexPayLines failed', e.message);
    return null;
  }
}
...
async function createNewCarDeal({ sheetModel, dealType, colours, breakup, regState, days = 15 }) {
  ...
  if (photoCount > 1) await refreshMedia(car.id).catch(() => {});
  try {
...
// Photos stream straight to the car's folder. They used to be held in memory —
// twelve phone photos is over a hundred megabytes of buffers on a 512MB box, and
// a single large upload was enough to restart the whole bot mid-upload.
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      const dir = path.join(MEDIA_ROOT, String(req.params.id || 'unfiled'));
    ...
    filename: (req, file, cb) => {
      ...
      cb(null, `${Date.now()}_${Math.floor(Math.random() * 1e4)}${ext}`);
    }
  }),
...
// Two keys open this door. The admin token does everything. The staff token —
// the one handed to whoever actually enters the cars — can add a car, upload and
// arrange its photos, and fix its details WHILE IT IS A DRAFT. It cannot publish,
// cannot unpublish, cannot delete, cannot touch a live listing or a loan scheme,
// and cannot send anything to a customer.
const STAFF_ALLOWED = [
  { m: 'GET', re: /^\/api\/inventory\// },
  { m: 'POST', re: /^\/api\/inventory\/cars$/ },
  // Staff may correct a live listing's details — the km a dealer got wrong, a
  // colour, a photo. What they may never touch on a live car is what it costs
  // and whether it is published.
  { m: 'PUT', re: /^\/api\/inventory\/cars\/[^/]+$/, detailsOnly: true },
  { m: 'POST', re: /^\/api\/inventory\/cars\/[^/]+\/(photos|cover|hero|enrich|webphotos)$/ },
  { m: 'POST', re: /^\/api\/inventory\/cars\/[^/]+\/photos\/[^/]+\/private$/ },
  { m: 'DELETE', re: /^\/api\/inventory\/cars\/[^/]+\/photos\/[^/]+$/, draftOnly: true }
];

function _staffMayPass(req) {
  const path = req.baseUrl ? req.baseUrl + req.path : req.path;
  const rule = STAFF_ALLOWED.find(r => r.m === req.method && r.re.test(path));
  if (!rule) return false;
  const idm = /^\/api\/inventory\/cars\/([^/]+)/.exec(path);
  const car = idm ? findCar(idm[1]) : null;
  if (rule.draftOnly) return !!car && car.status !== 'live';
  if (rule.detailsOnly) {
    if (!car) return false;
    if (car.status !== 'live') return true;         // a draft is theirs to shape
    // On a live car, refuse a CHANGE to the fields that decide money and
    // visibility — not their mere presence. Studio always sends the whole car
    // back, so checking for the key alone blocked every save staff attempted.
    const OWNER_ONLY = ['price', 'status', 'type', 'deal', 'expiresAt', 'tradePrice', 'consignment', 'loanSchemeIds'];
    const body = req.body || {};
    // Compare what the values MEAN, not how they were written. Studio fills the
    // whole form in: price arrives as {asking, offer, onRoad} with zeros where
    // the stored car simply has no key, deal as an object of empty strings, and
    // tradePrice as 0. Literal comparison called all of that a change and
    // refused every save.
    const n = v => Number(v) || 0;
    const str = v => String(v == null ? '' : v).trim();
    const same = (key, sent, stored) => {
      if (key === 'price') {
        const a = sent || {}, b = stored || {};
        return n(a.asking) === n(b.asking) && n(a.offer) === n(b.offer) && n(a.onRoad) === n(b.onRoad);
      }
      if (key === 'deal') {
        const a = sent || {}, b = stored || {};
        return ['headline', 'discount', 'exchangeBonus', 'freebies', 'validTill']
          .every(f => str(a[f]) === str(b[f]));
      }
      if (key === 'tradePrice') return n(sent) === n(stored);
      if (key === 'consignment') return !!sent === !!stored;
      if (key === 'loanSchemeIds') {
        const a = [...(sent || [])].sort(), b = [...(stored || [])].sort();
        return a.length === b.length && a.every((x, i) => String(x) === String(b[i]));
      }
      return str(sent) === str(stored);
    };
    return !OWNER_ONLY.some(k => Object.prototype.hasOwnProperty.call(body, k) && !same(k, body[k], car[k]));
  }
  return true;
}
...
router.use('/api/inventory', (req, res, next) => {
  ...
  if (!admin) return res.status(503).json({ ok: false, error: 'INVENTORY_ADMIN_TOKEN not configured' });
  const sent = String(req.headers['x-admin-token'] || '').trim();
  if (sent && sent === admin) { req.invRole = 'admin'; return next(); }
  if (staff && sent && sent === staff) {
    req.invRole = 'staff';
    if (_staffMayPass(req)) return next();
    return res.status(403).json({
      ok: false,
      error: 'Your login can add cars and fix their details. Prices, publishing and deleting are for the owner.'
    });
  }
  return res.status(401).json({ ok: false, error: 'Unauthorized' });
});

// The page asks who it is talking to so it can hide what this login cannot do.
router.get('/api/inventory/whoami', (req, res) => {
  res.json({ ok: true, role: req.invRole || 'admin' });
});
...
router.get('/api/inventory/storage', (req, res) => {
  ...
  res.json({
    ...
    // Where WhatsApp is told to fetch photos from. If this host does not serve
    // /media_store, every photo the bot sends silently fails to load.
    publicBaseUrl: String(_config.PUBLIC_BASE_URL || process.env.PUBLIC_BASE_URL || process.env.RENDER_EXTERNAL_URL || '') || null,
    siteUrl: SITE_URL(),
    staffLoginEnabled: !!String(process.env.INVENTORY_STAFF_TOKEN || '').trim(),
    storeReady: storeReady(),
    wabaId: global.__wabaId || null,
    disk: _diskUsage(),
    // Whether a completed intake can reach the inventory sheet at all.
    sheetWebhook: !!String(process.env.INVENTORY_SHEET_WEBHOOK_URL || process.env.GOOGLE_SHEET_WEBHOOK_URL || '').trim(),
    nextId: `MC-${String(db.seq).padStart(4, '0')}`,
    env: {
...
// A refused upload still leaves whatever multer managed to write on disk, and an
// orphan file in a car's folder ends up in its reel — so clear them out and say
// plainly what went wrong instead of returning a 500.
function uploadPhotos(req, res, next) {
  upload.array('photos', 12)(req, res, err => {
    ...
    if (!tooBig) console.warn('Inventory: photo upload failed', err.message);
    return res.status(tooBig ? 413 : 400).json({
      ok: false,
      error: tooBig ? 'That photo is over 8MB — please send a smaller one.' : `Upload failed: ${err.message}`
    });
  });
}

router.post('/api/inventory/cars/:id/photos', uploadPhotos, (req, res) => {
  ...
  refreshMedia(req.params.id);
  res.json({ ok: true, car: out.car, added: out.added });
});

// Rebuild the shareable reel on demand (Studio 🎬 button).
// Started in the background and answered immediately: a render is ~100s on the
// live box, far longer than a proxy will hold a request open. Poll the reel URL
// (or GET the car) to see when it lands.
router.post('/api/inventory/cars/:id/video', (req, res) => {
  try {
    if (!_config.dealVideo) return res.status(503).json({ ok: false, error: 'video engine not configured' });
    const car = findCar(req.params.id);
    if (!car) return res.status(404).json({ ok: false, error: 'Not found' });

    _config.dealVideo.build(car, { force: true })
      .then(r => {
        if (r && r.ok) console.log('Inventory: reel ready', car.id, `${r.seconds}s`, `${Math.round(r.bytes / 1024)}KB`);
        else console.warn('Inventory: reel failed', car.id, r && r.error);
      })
      .catch(e => console.warn('Inventory: reel threw', car.id, e && e.message));

    return res.status(202).json({
      ok: true, started: true, id: car.id,
      url: _config.dealVideo.urlFor(car.id),
      note: 'rendering in the background — poll the url in about two minutes'
    });
  } catch (e) {
...
// Hide (or restore) one photo from every customer-facing surface — the escape
// hatch when the document check gets one wrong in either direction.
router.post('/api/inventory/cars/:id/photos/:file/private', (req, res) => {
  const car = setPhotoPrivate(req.params.id, req.params.file, req.body?.private !== false);
  if (!car) return res.status(404).json({ ok: false, error: 'Not found' });
  res.json({ ok: true, car });
});
...
router.put('/api/inventory/loan-schemes/:id', (req, res) => {
  ...
  const b = req.body || {};
  // 'plans' decides which columns a listing shows — leaving it out of this list
  // meant Flex Pay could never be switched on for a scheme, and the update said
  // 200 while ignoring the field.
  for (const k of ['bank', 'roi', 'maxTenure', 'minDownPct', 'processingFee', 'notes', 'bulletPct', 'balloonPct', 'extensionTenure', 'surakshaPct', 'surakshaAmount', 'plans', 'balloonPreset', 'balloonCategory', 'defaultFor']) {
...
router.get('/api/vehyra/inventory', (req, res) => {
  ...
  res.json({
    ok: true,
    cars: cars.map(c => {
      const lean = _leanCar(c);
      if (_config.dealVideo && _config.dealVideo.existsFor(c.id)) lean.hasReel = true;
      return lean;
    })
  });
});

// The site's Pre-Owned page has always called this, and it never existed on
// this side — so "mg" returned "No pre-owned cars found" while two MG Hectors
// sat published. Live used listings, in the shape those cards read.
router.post('/api/vehyra/used', (req, res) => {
  const raw = String((req.body && req.body.query) || '').trim();
  const bodies = require('./bodyTypes.cjs');
  const body = bodies.askedBody(raw);
  // "suv under 15 lakh" is a body style and a ceiling, not a model name.
  let budget = 0;
  try {
    const vs = require('./variantSearch.cjs');
    budget = typeof vs.parseBudget === 'function' ? vs.parseBudget(raw) : 0;
  } catch (e) {}
  let q = raw.toLowerCase();
  if (body) q = q.replace(/\b(suvs?|muvs?|mpvs?|sedans?|saloon|hatchbacks?|hatch|crossover|7\s*seater|seven\s*seater|compact suv|sub compact suv)\b/gi, ' ');
  if (budget) {
    try {
      const vs = require('./variantSearch.cjs');
      if (typeof vs.stripBudgetWords === 'function') q = vs.stripBudgetWords(q);
    } catch (e) {}
  }
  q = q.replace(/\s+/g, ' ').trim();

  const cars = db.cars
    .filter(c => c.status === 'live' && c.type !== 'new')
    .filter(c => bodies.isBody(`${carTitle(c)} ${c.make || ''} ${c.model || ''}`, body))
    .filter(c => !budget || (_effectivePrice(c) && _effectivePrice(c) <= budget * 1.1))
    .filter(c => !q || `${carTitle(c)} ${c.make || ''} ${c.model || ''} ${c.variant || ''} ${c.colour || ''} ${c.fuel || ''}`.toLowerCase().includes(q))
    .map(c => {
      const photos = publicPhotos(c);
      ...
      return {
        ...
        colour: c.colour || '',
        owners: c.owners || '',
        // The browse filters need a body style, and no listing carries one.
        body: bodies.bodyOf(`${carTitle(c)} ${c.make || ''} ${c.model || ''}`) || '',
        ownerNumber: Number(String(c.owners || '').replace(/[^\d]/g, '')) || undefined,
        registration: c.registration || '',
        price: _effectivePrice(c),
        cover: cover ? cover.url : null,
...
router.get('/api/vehyra/inventory/:id', (req, res) => {
  ...
  const {
    ...
    sourceDealerName, sourceDealerPhone, sourceDealerCity, tradePrice, consignment,
    ...pub
  } = car;
  const schemes = _schemesForCar(car);
  const price = _effectivePrice(car);
  res.json({
    ...
    car: {
      ...pub,
      photos: publicPhotos(car),
      title: carTitle(car),
      // The site plays the reel through its own media proxy, so it only needs to
      // know one has been rendered — the backend host is never exposed.
      reel: (_config.dealVideo && _config.dealVideo.existsFor(car.id))
        ? `/api/vehyra/media/inventory/${car.id}/reel.mp4`
        : null,
      schemes,
...
function searchLive(query) {
  ...
  return db.cars
    ...
    .map(c => {
      ...
      const cover = publicPhotos(c).find(p => p.cover) || publicPhotos(c)[0];
      return {
        id: c.id,
        brand: c.make,
        model: carTitle(c),
...
router.post('/api/inventory/cars/:id/webphotos', async (req, res) => {
  try {
    ...
    car.updatedAt = new Date().toISOString();
    _save();
    if (added > 1) await refreshMedia(car.id).catch(() => {});
...
module.exports = {
  publishCar, publishAndReply, sendPublishPrompt, publicPhotos, setPhotoPrivate, storeReady,
  editFromText, editAndReply,
  init,
  router,
  handleButton,
  sendCarCard,
