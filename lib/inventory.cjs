...
// ---------------- helpers ----------------
function _nextCarId() {
  // seq is the counter, but it only has to be wrong once — a store written
  // without it reloads as seq 1 and the next "new" car would be handed MC-0001,
  // the id of a car that already exists. findCar returns the first match, so
  // every later edit, photo and publish would land on the wrong listing. Skip
