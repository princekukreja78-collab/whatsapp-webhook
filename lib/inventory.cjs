...
async function sendShareableCard(to, car) {
  ...
  // Prefer the reel when one has been rendered: it carries the same VehYra card
  // as its closing frame, so nothing is lost, and a video earns far more watch
  // time in a family group than a still. Falls straight back to the card image.
  if (_config.dealVideo && _config.dealVideo.existsFor(car.id)) {
    ...
    if (v && (v.messages || (v.ok && v.resp?.messages))) return true;
    console.warn('Inventory: reel send failed, falling back to card —', v && v.error);
  }
