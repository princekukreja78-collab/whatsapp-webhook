...
const STAFF_ALLOWED = [
  ...
  { m: 'PUT', re: /^\/api\/inventory\/cars\/[^/]+$/, draftOnly: true },
  { m: 'POST', re: /^\/api\/inventory\/cars\/[^/]+\/(photos|cover|hero|enrich|webphotos)$/, draftOnly: true },
  { m: 'POST', re: /^\/api\/inventory\/cars\/[^/]+\/photos\/[^/]+\/private$/, draftOnly: true },
...
function _staffMayPass(req) {
  ...
  if (!rule.draftOnly) return true;
  // Anything scoped to one car has its id as the first segment after /cars/.
  const idm = /^\/api\/inventory\/cars\/([^/]+)/.exec(path);
  const car = idm ? findCar(idm[1]) : null;
  return !!car && car.status !== 'live';
}
...
router.use('/api/inventory', (req, res, next) => {
  ...
  if (staff && sent && sent === staff) {
    ...
    return res.status(403).json({
      ok: false,
      error: 'Your login can add and edit a car while it is a draft. Publishing, deleting and live listings are for the owner.'
    });
  }
