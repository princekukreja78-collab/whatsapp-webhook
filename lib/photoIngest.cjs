...
async function handlePhoto(from, name, imageId, caption) {
  ...
  // A half-finished listing from earlier used to swallow every new photo in
  // silence — send a car, get nothing back, with no way to tell whether the bot
  // was working. Anything older than ten minutes is abandoned, and photos for a
  // car still being listed are acknowledged rather than absorbed quietly.
  const existing = pendingPhotos.get(from);
  const STALE_MS = 10 * 60 * 1000;
  if (existing && Date.parse(existing.createdAt || 0) && Date.now() - Date.parse(existing.createdAt) > STALE_MS) {
    _note(from, 'stale', `dropped ${existing.id} — older than 10 minutes`);
    pendingPhotos.delete(from);
  }
  const live = pendingPhotos.get(from);
  if (live && live.stage === 'AWAITING_CONFIRM') {
    // Save additional photo to existing pending entry
    try {
      let localUrl = null;
      if (_config.mediaStore) {
        localUrl = await _config.mediaStore.saveMedia(imageId, 'pending_' + live.id, from, live.id);
      }
      if (!live.extraPhotos) live.extraPhotos = [];
      live.extraPhotos.push(localUrl || imageId);
      console.log(`PhotoIngest: added extra photo to pending ${live.id} (total: ${1 + live.extraPhotos.length})`);
    } catch (e) {}
    if (!live.extraNoticeSent) {
      live.extraNoticeSent = true;
      await _config.waSendText(from,
        `📎 Added to *${live.id}* — the car you're listing now. Reply with its details when you're ready, or send *NEW CAR* to start a different one.`);
    }
    return true;
  }
...
async function _processBatch(from) {
  ...
  // Photos of an actual car are never a price sheet or a price list. Declared
  // here because every branch below asks the question.
  ...
  // Photos of a car are not a price list. A Defender's own photographs were
  // read as one, and three variants were invented at 95 lakh, 1 crore and 1.2
  // crore with a button offering to apply them to every quote.
  if (!looksUsed && !looksLikeACar && _isDealAdmin(from) && _config.priceSync) {
    try {
  ...
  // Ask, do not guess. Every misfile so far — a used Evoque read as a price
  // list, a GT 63 pushed into the new-car deal flow — came from the bot
  // deciding the category on its own.
  pendingPhotos.set(from, {
    id: batch.carId,
  ...
  const seen = [analysis.year ? `~${analysis.year}` : '', analysis.brand, analysis.model, analysis.variant]
    .filter(Boolean).join(' ') || 'this car';
  if (typeof _config.waSendRaw !== 'function') {
    // Never leave the dealer with silence because a sender is missing.
    await _config.waSendText(from,
      `*${batch.photos.length} photo(s) received*\n\nLooks like *${seen}*.\n\n` +
      `Reply *USED*, *NEW* or *DEMO* to tell me what to list it as.`);
    return;
  }
  await _config.waSendRaw({
    ...
    interactive: {
      type: 'button',
      body: { text: `*${batch.photos.length} photo(s) received*\n\nLooks like *${seen}*.\n\nWhat should I list it as?` },
      footer: { text: 'VehYra by MR. CAR' },
      action: {
        buttons: [
          { type: 'reply', reply: { id: `INV_CAT_USED_${batch.carId}`, title: '🚗 Pre-owned' } },
          { type: 'reply', reply: { id: `INV_CAT_NEW_${batch.carId}`, title: '🆕 New car' } },
          { type: 'reply', reply: { id: `INV_CAT_DEMO_${batch.carId}`, title: '🔑 Demo car' } }
        ]
      }
    }
  });
...
/**
 * The dealer picked a category. Ask for the details that category needs.
 */
async function chooseCategory(from, category) {
  const pending = pendingPhotos.get(from);
  if (!pending || pending.stage !== 'AWAITING_CATEGORY') return false;
  const cat = ['new', 'used', 'demo'].includes(category) ? category : 'used';
  pending.category = cat;
  pending.stage = 'AWAITING_CONFIRM';
  pendingPhotos.set(from, pending);
  _note(from, 'category', `${pending.id} → ${cat}`);

  const a = pending.analysis || {};
  const BUSINESS_NAME = process.env.MRCAR_BUSINESS_NAME || 'MR. CAR, Ashok Vihar';
  const label = cat === 'new' ? 'New car' : cat === 'demo' ? 'Demo car' : 'Pre-owned';
  let msg = `Hi, this is an assistant from *${BUSINESS_NAME}*.\n\n`;
  msg += `*${label} — ID: ${pending.id}*\n\n`;
  msg += `Our system detected:\n`;
  if (a.brand) msg += `Brand: *${a.brand}*\n`;
  if (a.model) msg += `Model: *${a.model}*\n`;
  if (a.variant) msg += `Variant: *${a.variant}*\n`;
  if (a.year) msg += `Year: *~${a.year}*\n`;
  if (a.color) msg += `Color: *${a.color}*\n`;
  if (a.fuel) msg += `Fuel: *${a.fuel}*\n`;
  if (a.rc && a.rc.regNumber) msg += `\n📄 *RC detected:* ${a.rc.regNumber}\n`;
  msg += `\nPlease confirm above and share:\n`;
  msg += `1. *Correct model & year* (if different)\n`;
  if (cat === 'used') {
    msg += `2. *KM driven*\n3. *Asking price*\n4. *Last price* (best offer)\n5. *Owner* (1st/2nd/3rd)\n6. *Registration city*\n`;
  } else if (cat === 'demo') {
    msg += `2. *KM run*\n3. *Asking price*\n4. *Last price* (best offer)\n5. *Registration city*\n`;
  } else {
    msg += `2. *Ex-showroom or on-road price*\n3. *Colours available*\n4. *Registration state*\n`;
  }
  msg += `\n_Just reply with the details in any format._`;
  await _config.waSendText(from, msg);
  return true;
}
  ...
  if (_config.inventory) {
    try {
      ...
      draftCar = _config.inventory.createFromIngest({
        entry: { ...entry, variant: pending.analysis.variant || '', category: pending.category || 'used' },
...
/** Abandon whatever car this dealer was part-way through listing. */
function resetPending(from) {
  pendingPhotos.delete(from);
  const b = photoBatch.get(from);
...
function hasPending(from) {
  const p = pendingPhotos.get(from);
  return !!(p && (p.stage === 'AWAITING_CONFIRM' || p.stage === 'AWAITING_DEAL_TYPE' || p.stage === 'AWAITING_CATEGORY'));
}
...
function _parseDetails(text, analysis) {
  ...
  const MODEL_BRAND = {
    ...
    'land cruiser': 'Toyota', 'lc 300': 'Toyota', lc300: 'Toyota', prado: 'Toyota',
    vellfire: 'Toyota', 'mini cooper': 'Mini', cooper: 'Mini', countryman: 'Mini',
    'v-class': 'Mercedes-Benz', 'g-class': 'Mercedes-Benz', maybach: 'Mercedes-Benz',
    panamera: 'Porsche', taycan: 'Porsche', 'gran coupe': 'BMW',
  ...
  // Some model names are ordinary words — a listing saying "DELHI CITY" or
  // "CITY REG" was read as a Honda City, and with Mini Cooper and Land Cruiser
  // missing from the table entirely there was nothing to outrank it. An
  // ambiguous name only counts when its make is named too, or when it appears
  // in the listing's own title line.
  const AMBIGUOUS = new Set(['city', 'venue', 'punch', 'safari', 'compass', 'defender', 'elevate', 'cooper']);
  ...
  for (const [mdl, brand] of Object.entries(MODEL_BRAND)) {
    ...
    if (!m) continue;
    if (AMBIGUOUS.has(mdl)) {
      const makeNamed = new RegExp(`\\b${brand.split(/[\s-]/)[0]}\\b`, 'i').test(t);
      const inTitle = new RegExp(`\\b${mdl}\\b`, 'i').test(firstLine);
      if (!makeNamed && !inTitle) continue;
    }
...
module.exports = {
  init, handlePhoto, handleReply, hasPending, isCollecting, captureDealText, intakeState, chooseCategory, resetPending,
