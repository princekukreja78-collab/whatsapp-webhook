...
async function _processBatch(from) {
  ...
  if (_isDealAdmin(from) && _config.priceSync) {
    try {
...
function _parseDetails(text, analysis) {
  ...
  if (!result.regCity) {
    ...
    const cm = t.match(/\b(dl|hr|up|pb|rj|mh|ka|ch|hp|uk|ga)\b\s*(?:reg|registration|number|no|\d)/i);
  ...
  const MODEL_BRAND = {
    ...
    magnite: 'Nissan', compass: 'Jeep', meridian: 'Jeep'
  };
  ...
  for (const [mdl, brand] of Object.entries(MODEL_BRAND)) {
    if (new RegExp(`\\b${mdl}\\b`, 'i').test(t)) {
      result.model = mdl.replace(/\b\w/g, c => c.toUpperCase());
      if (!result.brand || result.brand.toLowerCase() !== brand.toLowerCase()) result.brand = brand;
      break;
    }
  }
