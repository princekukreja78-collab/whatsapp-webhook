...
async function _analyzeCarPhotos(photos, caption) {
  const result = { brand: '', model: '', variant: '', year: '', color: '', fuel: '', condition: '', rc: null };
  ...
  try {
    ...
    const resp = await _config.openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a car identification expert for an Indian used-car dealer. You get MULTIPLE photos of one car (exterior/interior shots, and possibly a photo of its RC — Registration Certificate card or Vahan/DigiLocker screenshot). Return ONLY a JSON object:
{"brand":"","model":"","variant":"","year":"","color":"","fuel":"","condition":"","rc":{"regNumber":"","regYear":"","fuel":"","cc":"","regCity":"","expiry":""}}
