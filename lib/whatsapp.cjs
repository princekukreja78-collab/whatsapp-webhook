...
// One service can answer more than one WhatsApp number — the staging number and
// the live one share this bot and its inventory, so nothing is lost to a
// separate environment. A reply must leave through the SAME number it arrived
// on, or the customer sees it come from a different business account, so the
// webhook stashes that number id for the life of the request.
const _numberContext = new AsyncLocalStorage();
function withPhoneNumberId(phoneNumberId, fn) {
  if (!phoneNumberId) return fn();
  return _numberContext.run(String(phoneNumberId), fn);
}
function currentPhoneNumberId() {
  return _numberContext.getStore() || _config.PHONE_NUMBER_ID;
}
// Called once at the top of a webhook request: everything that request goes on
// to await inherits the number, so a reply cannot leave through the wrong one.
function setOutboundNumberId(phoneNumberId) {
  if (phoneNumberId) _numberContext.enterWith(String(phoneNumberId));
}
...
async function waSendRaw(payload) {
  const numberId = currentPhoneNumberId();
  if (!_config.META_TOKEN || !numberId) {
...
// Plain video by URL. Unlike an interactive message this one forwards, which is
// the whole point of the deal reel — a customer passes it to family as-is.
async function waSendVideoLink(to, videoUrl, caption = "") {
  const payload = {
    messaging_product: "whatsapp",
    to,
    type: "video",
