...
async function buildUsedCarQuoteFreeText({ query, from }) {
  ...
  // A number only counts as a budget when it carries a unit ("12 lakh"), follows a
  // budget word ("under 12"), or is a plain large amount. Without this, model names
  // that contain digits — A4, Q7, X5, 3 Series — were read as budgets and the
  // customer got a price list instead of the car they asked for.
  const mUnit = qLower.match(/\b(\d+(?:\.\d+)?)\s*(crore|cr|lakh|lakhs|lac|lacs|l)\b/);
  const mWord = qLower.match(/\b(?:under|below|within|upto|up to|max|budget|around|about|near|approx)\s*(?:rs\.?|₹)?\s*(\d+(?:\.\d+)?)\b/);
  const mPlain = qLower.match(/\b(\d{6,8})\b/);
  if (mUnit) {
    const n = parseFloat(mUnit[1]);
    budgetRs = /^(crore|cr)$/.test(mUnit[2]) ? n * 10000000 : n * 100000;
  } else if (mWord) {
    const n = parseFloat(mWord[1]);
    budgetRs = n < 1000 ? n * 100000 : n;
  } else if (mPlain) {
    const v = Number(mPlain[1]);
    if (v >= 100000) budgetRs = v;
  }
...
global.lastUsedCarList.set(from, {
  ...
  // A price correction forwarded on WhatsApp beats the sheet — applied before the
  // loan amount and both EMI options are worked out.
  let benefitLine = null;
  try {
    if (_config.priceSync) {
      const ov = _config.priceSync.lookup(make, model, sub);
      if (ov) {
        if (ov.onRoad) expected = ov.onRoad;
        else if (ov.exShowroom) expected = ov.exShowroom;
        benefitLine = _config.priceSync.discountLine(ov);
        console.log('USED_PRICE_OVERRIDE_APPLIED:', { make, model, sub, expected });
      }
    }
  } catch (e) {
    console.warn('used price override skipped:', e && e.message ? e.message : e);
  }
  // Shown on every pre-owned quote too, so the column never disappears.
...
const bulletSim = simulateBulletPlan({
  ...
  // OPTION 3 — FLEX PAY / BALLOON on the used-car table
  try {
    if (_config.inventory && typeof _config.inventory.flexPayLines === 'function') {
      const flex = _config.inventory.flexPayLines({ amount: loanAmt, roi: USED_CAR_ROI_INTERNAL, months: 48, used: true });
      if (flex && flex.length) {
        lines.push('');
    ...
    console.warn('USED CAR FLEX PAY ERROR:', e && e.message ? e.message : e);
  }
