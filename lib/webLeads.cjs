...
async function captureLead(body = {}) {
  ...
  // 3. Follow-up — the site's warmest lead used to get no touch at all
  if (_config.followUp && typeof _config.followUp.addToQueue === 'function') {
    try {
      _config.followUp.addToQueue({
        phone, name, car, budget: lead.budget, source: lead.source
      });
    } catch (e) { console.warn('WebLeads: follow-up enqueue failed', e.message); }
  }
