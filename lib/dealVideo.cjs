...
// Chosen on measured opening level, not taste: of the four beds this is the only
// one that neither ramps in over six seconds nor drops out at three, and it runs
// long enough not to loop inside a reel. Override per environment with
// DEAL_VIDEO_MUSIC=<name>, or per call with opts.music.
const DEFAULT_TRACK = 'morning-parcel';

function resolveMusic(explicit) {
...
async function build(car, opts = {}) {
  ...
  try {
    ...
    const music = resolveMusic(opts.music);
    if (music) {
    ...
    cleanup();
    return { ok: true, path: outPath, url: urlFor(car.id), seconds: Math.round(seconds * 10) / 10, bytes, photos: photos.length, card: hasCard, music: !!music };
  } catch (e) {
    cleanup();
