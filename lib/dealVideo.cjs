...
function buildFilter(nPhotos, hasCard) {
  const parts = [];
  ...
  for (let i = 0; i < nPhotos; i++) {
    ...
    parts.push(
      ...
      `crop=${W * 2}:${H * 2},boxblur=30:1,eq=brightness=-0.10:saturation=0.85[bg${i}]`,
      `[${i}:v]scale=${W * 2}:-2:flags=lanczos[fg${i}]`,
      `[bg${i}][fg${i}]overlay=(W-w)/2:(H-h)/2[cmp${i}]`,
      // zoompan emits d frames for EVERY input frame, so each photo is fed as a
      // single still (no -loop) — looping it here multiplies the clip length.
      `[cmp${i}]zoompan=z='${z}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':` +
      `d=${frames}:s=${W * 2}x${H * 2}:fps=${FPS},scale=${W}:${H}:flags=lanczos,` +
      `setsar=1,scale=out_range=tv,format=yuv420p[v${i}]`
    );
  }

  // end card: 1080x1350 art, centred on a blurred fill of itself
  if (hasCard) {
    const ci = nPhotos;
    parts.push(
      `[${ci}:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},` +
      `boxblur=40:1,eq=brightness=-0.22:saturation=0.7[cbg]`,
      `[${ci}:v]scale=${W}:-2:flags=lanczos[cfg]`,
      `[cbg][cfg]overlay=(W-w)/2:(H-h)/2,setsar=1,scale=out_range=tv,format=yuv420p,` +
      `fps=${FPS}[v${ci}]`
    );
  }

  // chain the crossfades
  const total = nPhotos + (hasCard ? 1 : 0);
  let cur = 'v0', offset = SEG - XF;
  for (let i = 1; i < total; i++) {
    const out = i === total - 1 ? 'vout' : `x${i}`;
    const dur = i === total - 1 && hasCard ? CARD : SEG;
    parts.push(`[${cur}][v${i}]xfade=transition=fade:duration=${XF}:offset=${offset.toFixed(3)}[${out}]`);
    cur = out;
    offset += dur - XF;
  }
  if (total === 1) parts.push(`[v0]null[vout]`);
...
async function build(car, opts = {}) {
  ...
  const { filter, seconds } = buildFilter(photos.length, !!cardPath);

  const args = ['-y', '-hide_banner', '-loglevel', 'error'];
  // photos: one still each — zoompan generates the frames and therefore the duration
  for (const p of photos) args.push('-i', p);
  // card: no zoompan on it, so it does need looping to hold on screen
  if (cardPath) args.push('-loop', '1', '-t', String(CARD), '-i', cardPath);
  ...
  if (opts.music && fs.existsSync(opts.music)) {
    const ai = photos.length + (cardPath ? 1 : 0);
    args.push('-map', `${ai}:a`, '-af', 'loudnorm=I=-16:TP=-1.5:LRA=11', '-shortest', '-c:a', 'aac', '-b:a', '128k');
  } else {
    args.push('-an');
  }
  args.push(
  ...
  log('rendering', car.id, photos.length, 'photos, card:', !!cardPath);
  const t0 = Date.now();
  const r = await run(FFMPEG, args, 300000);
  if (cardPath) { try { fs.unlinkSync(cardPath); } catch (e) {} }
  ...
  return { ok: true, path: outPath, url: urlFor(car.id), seconds: Math.round(seconds * 10) / 10, bytes, photos: photos.length, card: !!cardPath };
}
...
module.exports = { init, build, existsFor, urlFor, pickPhotos, _buildFilter: buildFilter };
