...
function pickPhotos(car, mediaRoot) {
  ...
  const ordered = [];
  const all = Array.isArray(car.photos) ? car.photos.slice() : [];
  // Paperwork (RC card, price sheet) is stored alongside the car photos. It must
  // never reach a reel — including via the unlisted-files sweep below.
  const blocked = new Set(all.filter(p => p && p.private && p.file).map(p => p.file));
  const listed = all.filter(p => !(p && p.private));
  listed.sort((a, b) => (b.cover ? 1 : 0) - (a.cover ? 1 : 0));
  for (const p of listed) {
    if (p && p.file && onDisk.has(p.file)) { ordered.push(p.file); onDisk.delete(p.file); }
  }
  for (const f of onDisk) { if (!blocked.has(f)) ordered.push(f); }
  ...
  try {
    ...
    if (music) {
      ...
      args.push('-stream_loop', '-1', '-i', music, '-map', '0:v', '-map', '1:a',
        ...
        '-t', String(seconds),
        // loudnorm resamples to 192kHz internally, and without an explicit rate
        // the encoder settled on 96kHz AAC — legal, but outside what Safari
        // decodes, so the reel played with a silent audio track.
        '-ar', '48000',
