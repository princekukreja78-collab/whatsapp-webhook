...
/**
 * Bing images fallback — DDG blocks some datacenter IPs (e.g. Render).
 */
async function searchImagesBing(query, count = 4) {
  try {
    const resp = await _config.fetch(
      `https://www.bing.com/images/search?q=${encodeURIComponent(query)}&qft=+filterui:imagesize-large`,
      { headers: { 'User-Agent': UA, 'Accept-Language': 'en-IN,en' } }
    );
    if (!resp.ok) return [];
    const html = await resp.text();
    const urls = [...html.matchAll(/"murl":"(https:[^"]+?\.(?:jpe?g|png))"/gi)]
      .map(m => m[1].replace(/\\u002f/gi, '/'));
    return [...new Set(urls)].slice(0, count);
  } catch (e) {
    console.warn('webImages: bing search failed', e.message);
    return [];
  }
}
...
async function fetchImages(query, count = 2) {
  let urls = await searchImages(query, count + 4);
  if (!urls.length) {
    console.log(`webImages: DDG empty for "${query}" — trying Bing`);
    urls = await searchImagesBing(query, count + 4);
  }
  console.log(`webImages: ${urls.length} candidate urls for "${query}"`);
  const out = [];
  for (const u of urls) {
