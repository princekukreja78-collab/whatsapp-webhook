...
// Sheets that match the SHEET_<NAME>_CSV_URL shape but are NOT new-car price lists.
// SHEET_USED_CSV_URL is the pre-owned stock sheet: auto-discovered as a brand called
// "USED", it put second-hand cars into the new-car pool, so "Fortuner" answered with
// two 2022 used Fortuners instead of the 15 real Toyota variants. The used-car flow
// reads that sheet directly via loadUsedSheetRows(), so it must stay out of here.
const NON_BRAND_SHEETS = new Set(['USED', 'LEADS', 'CONTACT', 'DEALER', 'INVENTORY']);
...
function _buildSheetUrls() {
  ...
  for (const [envKey, value] of Object.entries(env)) {
    ...
    if (NON_BRAND_SHEETS.has(brandKey)) continue;
    if (!urls[brandKey]) {
...
// Toyota's price list is the first tab of this published sheet. Kept here as a
// fallback because SHEET_TOYOTA_CSV_URL has historically pointed at the leads
// spreadsheet, which silently broke every Toyota quote and search.
const TOYOTA_PUBLISHED_CSV =
  'https://docs.google.com/spreadsheets/d/e/2PACX-1vT1fxuSfv5DKtzjm2icH03KsOlHtFuv0cUxTPTkEBc5bpPWpZgW8LcxzMafFcDcmEvGfrn3U_RyrDDg/pub?output=csv';

/**
 * Guard the brand→URL map: a price-list URL must be a published (/d/e/…/pub) CSV.
 * Anything else — e.g. an /export link to the leads spreadsheet — is replaced.
 */
function _repairSheetUrls(urls) {
  try {
    const isPublished = u => /\/d\/e\/[^/]+\/pub/.test(String(u || ''));
    if (!isPublished(urls.TOYOTA)) {
      console.warn('Pricing: TOYOTA sheet URL missing or not a published price list — using the known Toyota sheet');
      urls.TOYOTA = TOYOTA_PUBLISHED_CSV;
    }
  } catch (e) { /* keep the original mapping on any surprise */ }
  return urls;
}

// ---------------- Delhi road tax band repair ----------------
// Delhi charges MV tax on ex-showroom in three bands. Electric vehicles are
// exempt (their sheet rows carry a flat nominal 7-10k, which is correct and must
// be left alone).
const DELHI_RT = { petrol: [0.04, 0.07, 0.10], diesel: [0.05, 0.0875, 0.125] };
const _rtBand = ex => (ex < 600000 ? 0 : ex < 1000000 ? 1 : 2);

// Delhi has withdrawn the blanket EV exemption above a price threshold. The rate
// is left unset on purpose: with a 3.1cr G 580 in the sheet, guessing a
// percentage would move a quote by tens of lakhs. Until DELHI_EV_ROAD_TAX_PCT is
// supplied, EV rows are passed through untouched exactly as before.
const EV_TAX_THRESHOLD = Number(process.env.DELHI_EV_TAX_ABOVE || 3000000);
const EV_TAX_PCT = process.env.DELHI_EV_ROAD_TAX_PCT ? Number(process.env.DELHI_EV_ROAD_TAX_PCT) : null;
const _isEv = fuel => /ELECTRIC|BEV|\bEV\b/.test(String(fuel || '').toUpperCase());

/**
 * Is this cell credible as a road tax at all?
 *
 * Some sheet cells hold something else entirely — 53 KIA diesel rows carry
 * 110% of ex-showroom in the UP column, an on-road figure pasted into a tax
 * column, which quoted a 12 lakh Sonet at 22 lakh in Noida. A number far
 * outside every Indian state's slab is not a tax we can convert; the honest
 * answer is to withhold that state rather than quote it or invent a rate.
 * EVs are exempt and legitimately carry a nominal fee, so they are excluded.
 */
function plausibleRoadTax(ex, tax, fuel) {
  const e = Number(ex) || 0, t = Number(tax) || 0;
  if (!e || !t) return false;
  if (/ELECTRIC|BEV|\bEV\b/.test(String(fuel || '').toUpperCase())) return true;
  const pct = (t / e) * 100;
  return pct >= 2 && pct <= 25;   // every state slab in India sits inside this
}

/**
 * Correct rows that were charged another BAND's rate for their own fuel — the
 * sub-10L cars billed at the above-10L rate, which over-quoted them by ~3% of
 * ex-showroom. Deliberately narrow: a value that matches no band rate at all is
 * left untouched and reported, because that is a data question, not a slip.
 * Returns the list of corrections for logging.
 */
function repairRoadTax(brand, tb) {
  const fixes = [];
  try {
    const h = tb.header.map(c => String(c || '').replace(/\n/g, ' ').trim());
    const iEx = h.findIndex(c => /Ex Showroom/i.test(c));
    const iFuel = h.findIndex(c => /FUEL/i.test(c));
    const iRt = h.findIndex(c => /Road Tax DELHI INDIVIDUAL/i.test(c));
    const iOn = h.findIndex(c => /On Road Price DELHI INDIVIDUAL/i.test(c));
    const iM = h.findIndex(c => /^MODEL$/i.test(c));
    const iV = h.findIndex(c => /^VARIANT$/i.test(c));
    if (iEx < 0 || iRt < 0 || iFuel < 0) return fixes;

    const num = v => Number(String(v ?? '').replace(/[^\d.-]/g, '')) || 0;

    for (const r of tb.data) {
      const ex = num(r[iEx]), rt = num(r[iRt]);
      if (!ex || !rt) continue;
      const fuel = String(r[iFuel] || '').toUpperCase();
      if (_isEv(fuel)) {
        // Exempt below the threshold; above it, only act once a rate is supplied.
        if (EV_TAX_PCT == null || ex <= EV_TAX_THRESHOLD) continue;
        const evExpected = Math.round(ex * (EV_TAX_PCT / 100));
        if (Math.abs(rt - evExpected) <= Math.max(500, ex * 0.002)) continue;
        const evDelta = evExpected - rt;
        r[iRt] = String(evExpected);
        if (iOn >= 0 && num(r[iOn])) r[iOn] = String(Math.round(num(r[iOn]) + evDelta));
        fixes.push({ brand, model: String(r[iM] || ''), variant: String(r[iV] || ''), ex, was: rt, now: evExpected, delta: evDelta, ev: true });
        continue;
      }
      const rates = /DIESEL/.test(fuel) ? DELHI_RT.diesel : DELHI_RT.petrol;
      const band = _rtBand(ex);
      const expected = Math.round(ex * rates[band]);
      if (Math.abs(rt - expected) <= Math.max(500, ex * 0.002)) continue;  // already right

      // only act when the value equals ANOTHER band's rate for this same fuel
      const slipped = rates.some((rate, i) => i !== band && Math.abs(rt - ex * rate) <= Math.max(500, ex * 0.002));
      if (!slipped) continue;

      const delta = expected - rt;
      r[iRt] = String(expected);
      if (iOn >= 0 && num(r[iOn])) r[iOn] = String(Math.round(num(r[iOn]) + delta));
      fixes.push({ brand, model: String(r[iM] || ''), variant: String(r[iV] || ''), ex, was: rt, now: expected, delta });
    }
  } catch (e) { /* never let a repair break pricing */ }
  return fixes;
}
...
async function loadPricingFromSheets() {
  ...
  const SHEET_URLS = _repairSheetUrls(_buildSheetUrls());
  const tables = {};
  // Every brand's sheet was fetched one after another, so a cold search waited
  // for the slowest chain of them all. They are independent — fetch together.
  const entries = Object.entries(SHEET_URLS).filter(([, url]) => url);
  const fetched = await Promise.all(entries.map(async ([brand, url]) => {
    try { return [brand, url, await fetchCsv(url)]; }
    catch (e) { return [brand, url, null]; }
  }));
  for (const [brand, url, rows] of fetched) {
    try {
      ...
      const fixes = repairRoadTax(brand, tables[brand]);
      if (fixes.length) {
        console.log(`Pricing: corrected Delhi road tax on ${fixes.length} ${brand} row(s) —`,
          fixes.map(f => `${f.model} ${f.variant} ${f.was}→${f.now}`).join('; '));
      }
    } catch (e) {
...
if (stateMatch) {
  ...
  // Build abbreviation lookup for common states
  const STATE_ABBREV_ONROAD = {
    'uttarpradesh': 'up', 'himachalpradesh': 'hp', 'haryana': 'hr',
    'maharashtra': 'mh', 'madhyapradesh': 'mp', 'tamilnadu': 'tn',
    'karnataka': 'ka', 'telangana': 'ts', 'rajasthan': 'rj',
    'punjab': 'pb', 'gujarat': 'gj', 'kerala': 'kl',
    'westbengal': 'wb', 'andhrapradesh': 'ap', 'chandigarh': 'ch'
  };
  const stateAbbr = STATE_ABBREV_ONROAD[normState] || '';
  const stateFirstWord = normState.match(/^[a-z]+/)?.[0] || normState;
  const stateTokens = [normState, stateAbbr, stateFirstWord].filter(Boolean);
  ...
  for (const k of keys) {
    ...
    if (normKey.includes('onroadprice') &&
        stateTokens.some(tok => tok && normKey.includes(tok))
    ) {
      return idxMap[k];
    }
  }
