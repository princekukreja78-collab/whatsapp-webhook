...
function lookup(brand, model, variant) {
  ...
  for (const o of store.overrides) {
    if (!o.key) continue;
    if (target === o.key || target.includes(o.key) || o.key.includes(target)) {
...
function discountLine(o) {
  ...
  if (o.discountVariable) return `🎁 *Discount: negotiable* — tell us your budget and we'll get you the best possible price.`;
  return null;
}
