...
async function handlePriceList(from, imageUrls) {
  ...
  // Nothing readable behind the numbers means the model supplied them. Say so
  // and stop — do not offer to apply invented prices to live quotes.
  if (!sheetAvailable) {
    await _config.waSendText(from,
      `📋 I could not read a price sheet in those photos, so I have not stored anything.\n\n` +
      `If this is a price list, send the sheet itself — a clear photo of the table with ex-showroom, TCS, insurance and road tax.`);
    return true;
  }
