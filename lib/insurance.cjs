...
const COVERAGE_EXPLAIN = {
  thirdParty:
    ...
    `• Legal liability for death or bodily injury to third party\n` +
    `• ₹15 Lakh Personal Accident cover for owner-driver\n\n` +
    `Does NOT cover: Your own car — no theft, fire, or accident damage\n\n` +
    `Best for: Old cars (10+ years), very tight budget\n` +
    `TP premium is fixed by IRDAI — same across all insurers`,

  comprehensive:
    `*Comprehensive (OD + TP)* — Most Popular\n\n` +
    `Covers everything in TP plus:\n` +
    `• *Own Damage* — accident, theft, fire\n` +
    `• *Natural calamities* — earthquake, storm, cyclone\n` +
    `• *Malicious damage* — vandalism, terrorism, riots\n` +
    `• *Personal Accident* — ₹15 Lakh for owner-driver\n\n` +
    `Does NOT cover:\n` +
    `• Wear & tear, mechanical/electrical breakdown\n` +
    `• Depreciation on parts (plastic, rubber, glass)\n` +
    `• Consumables (oil, coolant, brake fluid)\n` +
    `• Engine damage from water ingression\n\n` +
    `Best for: Most car owners, daily use vehicles`,

  zeroDep:
    `*0 Dep + Consumables + Engine Cover* — Full Protection\n\n` +
    `Everything in Comprehensive PLUS these add-ons:\n\n` +
    `• *Zero Depreciation* — No deduction on parts during claim. Without this, you pay 30-50% of rubber/plastic/glass parts from pocket\n\n` +
    `• *Engine Secure* — Engine damage from water ingression, oil leakage, hydrostatic lock. Without this, engine repair (₹1-5L) is on you\n\n` +
    `• *Consumable Cover* — Engine oil, brake fluid, AC gas, coolant, nuts, bolts, bearings\n\n` +
    `• *Tyre Secure* — Tyre damage during accident\n\n` +
    `• *Key Replacement* — Lost/stolen keys (up to ₹65,000)\n\n` +
    `• *Emergency Transport & Hotel* — Towing + hotel if breakdown away from home (₹25K/accident, ₹50K/year)\n\n` +
    `• *Personal Belongings* — Theft from car (up to ₹50,000)\n\n` +
    `• *Road Side Assistance (RSA)* — 24/7 breakdown help\n\n` +
    `Best for: Cars under 5 years, expensive cars, complete peace of mind`,

  rti:
    `*Return to Invoice (RTI)*\n\n` +
    `In case of *total loss or theft*, you get the *full invoice price* back — not just depreciated IDV.\n\n` +
    `Example:\n` +
    `Car bought at ₹15L → After 3 years, IDV drops to ₹9L\n` +
    `Without RTI: You get ₹9L\n` +
    `With RTI: You get ₹15L (full invoice)\n\n` +
    `*Important Rules:*\n` +
    `• RTI continues for up to *6 years* if renewed without any break\n` +
    `• Even *1 day gap* in renewal = RTI lost permanently\n` +
    `• Must be taken from Year 1 and renewed continuously\n` +
    `• Cannot be added later if not taken initially\n\n` +
    `Best for: New cars, high value vehicles in first 5-6 years`,

  ncb:
    `*No Claim Bonus (NCB)*\n\n` +
    `Discount earned for each claim-free year:\n` +
    `• After 1st year: *20% off*\n` +
    `• After 2nd year: *25% off*\n` +
    `• After 3rd year: *35% off*\n` +
    `• After 4th year: *45% off*\n` +
    `• After 5th year+: *50% off* (maximum)\n\n` +
    `On a ₹37,000 OD premium, 50% NCB saves ₹18,500!\n\n` +
    `*NCB resets to 0%* if you make any claim\n` +
    `*NCB is lost* if policy lapses more than 90 days\n\n` +
    `Tip: For small damages (under ₹10K), pay from pocket to protect NCB. The discount over years is much more than the repair cost.`,

  claimProcess:
    `*How to Claim — Simple Steps*\n\n` +
    `*At Network Garage (Cashless):*\n` +
    `1. Call insurer helpline (24/7)\n` +
    `2. Drive/tow to nearest network garage\n` +
    `3. Garage inspects damage\n` +
    `4. Repair done — direct cashless settlement\n\n` +
    `*At Any Garage (Reimbursement):*\n` +
    `1. Call insurer to initiate claim\n` +
    `2. Surveyor assesses damage\n` +
    `3. Submit docs (RC, DL, FIR if needed)\n` +
    `4. Get repair done — submit bills\n` +
    `5. Reimbursement in 7-15 days\n\n` +
    `*Documents:* RC copy, Driving License, Policy copy, FIR (theft/major accident), Repair bills & photos`
};
...
async function _handleCoverage(from, id, state) {
  ...
  if (/\b(explain|difference|compare|samjhao|batao|kya hai)\b/i.test(id)) {
    // Send in parts to avoid WhatsApp message limit
    await _config.waSendText(from, `*Insurance Guide — Part 1/3*\n\n` + COVERAGE_EXPLAIN.thirdParty + `\n\n---\n\n` + COVERAGE_EXPLAIN.comprehensive);
    await _config.waSendText(from, `*Insurance Guide — Part 2/3*\n\n` + COVERAGE_EXPLAIN.zeroDep);
    let explanation = `*Insurance Guide — Part 3/3*\n\n`;
    explanation += COVERAGE_EXPLAIN.rti + `\n\n---\n\n`;
    explanation += COVERAGE_EXPLAIN.ncb + `\n\n---\n\n`;
    explanation += COVERAGE_EXPLAIN.claimProcess;
