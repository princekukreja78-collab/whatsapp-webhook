...
function benefitsFor({ brand, model, variant, discount, insuranceBenefit }) {
  ...
  // The same two benefits the WhatsApp quote offers
  if (d) list.push({ label: 'Discount benefit', value: d, kind: 'amount' });
...
router.post('/api/vehyra/variant-detail', express.json(), async (req, res) => {
  ...
  try {
    const { brand = '', model = '', variant = '', price = 0, discount = 0, insuranceBenefit = 0 } = req.body || {};
