...
function emiPlansFor(price) {
  if (!_config.inventory || typeof _config.inventory.allSchemes !== 'function') return [];
  const schemes = _config.inventory.allSchemes();
  return schemes.map(s => ({
    scheme: {
      bank: s.bank, roi: s.roi, minDownPct: s.minDownPct,
      plans: s.plans || ['reducing', 'bullet'],
      balloonPct: s.balloonPct, extensionTenure: s.extensionTenure
    },
    rows: _config.inventory.emiRows(price, s)
  }));
}

// ---------------- benefits (what we call the discount) ----------------
function benefitsFor({ brand, model, variant, discount }) {
  const list = [];
  const d = num(discount);
  if (d) list.push({ label: 'Dealer benefit on this variant', value: d, kind: 'amount' });

  if (_config.priceSync) {
    const o = _config.priceSync.lookup(brand, model, variant);
    if (o) {
      if (o.discount && o.discount !== d) list.push({ label: 'Current offer', value: o.discount, kind: 'amount' });
      else if (o.discountVariable) list.push({
        label: 'Additional benefit',
        text: 'Negotiable — tell us your budget and we will get you the best possible price',
        kind: 'negotiable'
      });
    }
  }
  if (!list.length) {
    list.push({
      label: 'Additional benefit',
      text: 'Negotiable — Mr. Car negotiates corporate, exchange and loyalty benefits on your behalf',
      kind: 'negotiable'
    });
  }
  return list;
...
router.post('/api/vehyra/variant-detail', express.json(), async (req, res) => {
  ...
  try {
    const { brand = '', model = '', variant = '', price = 0, discount = 0 } = req.body || {};
    ...
    res.json({
      ...
      benefits: benefitsFor({ brand, model, variant, discount }),
      emiTables: num(price) ? emiPlansFor(num(price)) : []
    });
  } catch (e) {
