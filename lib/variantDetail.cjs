...
function benefitsFor({ brand, model, variant, discount, insuranceBenefit }) {
  ...
  if (d) {
    ...
    list.push({
      ...
      // Manufacturer schemes change month to month and most are eligibility
      // gated, so the standing line promises the conversation, not a figure.
      text: negotiable
        ? 'Manufacturer schemes vary periodically — your exact benefit is confirmed upon finalisation. Tell us your budget and we will get you the best possible price'
        : 'Manufacturer schemes vary periodically — corporate, exchange and loyalty benefits are confirmed upon finalisation',
      kind: 'negotiable'
    });
  }
