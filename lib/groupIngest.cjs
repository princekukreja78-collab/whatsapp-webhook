...
/**
 * Check if a number is a registered dealer.
 * Dealer numbers are treated differently:
 *   - Car listing message → ingest silently (no reply)
 *   - Normal message (hi, question, enquiry) → reply as normal customer
 */
function isDealerNumber(from) {
  if (!from) return false;
  const dealerList = (_config.DEALER_NUMBERS || '').split(',').map(s => s.trim()).filter(Boolean);
  return dealerList.includes(from);
}

/**
 * Detect if a message looks like a car listing (inventory post).
 * Car listings typically contain: year + price OR multiple car keywords.
 */
function _looksLikeCarListing(text) {
  if (!text || text.length < 15) return false;
  const t = text.toLowerCase();

  // Greeting/question/customer intent = NOT a listing
  if (/^(hi|hello|hey|namaste|enquiry|help|start|menu|loan|emi|finance|yes|no|ok|thanks|thank|book|interested|price|compare|budget)\b/.test(t)) return false;
  if (t.endsWith('?')) return false;
  if (t.includes('?')) return false; // any question mark anywhere
  if (t.split(/\s+/).length <= 4) return false;
  // Common customer phrases
  if (/\b(i want|i need|looking for|show me|tell me|what is|how much|kitna|chahiye|batao|dikhao)\b/.test(t)) return false;

  let signals = 0;

  // Year mention (2018-2026)
  if (/\b20[1-2]\d\b/.test(t)) signals++;

  // Price mention
  if (/(\d+(\.\d+)?)\s*(lakh|lac|l\b|₹)/i.test(t)) signals++;
  if (/₹\s*[\d,]+/.test(t)) signals++;

  // KM mention
  if (/\d+\s*(km|kms|kilometer)/i.test(t)) signals++;

  // Fuel/transmission
  if (/\b(diesel|petrol|cng|automatic|manual|at|mt)\b/i.test(t)) signals++;

  // Owner mention
  if (/\b(1st|2nd|3rd|first|second|third)\s*owner/i.test(t)) signals++;

  // Color
  if (/\b(white|black|silver|grey|gray|red|blue|brown)\b/i.test(t)) signals++;

  // Car brand/model names (strong signal)
  const carWords = ['fortuner', 'creta', 'innova', 'thar', 'xuv', 'swift', 'city', 'verna',
    'seltos', 'nexon', 'harrier', 'safari', 'hycross', 'scorpio', 'brezza', 'baleno',
    'bmw', 'mercedes', 'audi', 'range rover', 'porsche', 'volvo', 'jaguar', 'lexus',
    'glc', 'gle', 'gla', 'x1', 'x3', 'x5', 'q3', 'q5', 'q7', 'cayenne', 'macan',
    'compass', 'wrangler', 'defender', 'discovery', 'fortuner', 'camry', 'civic',
    'kushaq', 'taigun', 'hector', 'astor', 'carnival', 'alcazar', 'tucson'];
  for (const w of carWords) {
    if (t.includes(w)) { signals += 2; break; }
  }

  // Need at least 3 signals to classify as a car listing
  return signals >= 3;
}
...
/**
 * Handle message from a dealer number.
 * If it looks like a car listing → ingest silently, return true (no reply).
 * If it looks like a normal message → return false (let normal flow handle it).
 * @returns {boolean} true = ingested silently, false = process as normal customer
 */
async function handleDealerMessage(from, msgText, senderName) {
  if (!isDealerNumber(from)) return false;

  // Check if this message looks like a car listing
  if (!_looksLikeCarListing(msgText)) {
    // Normal message from dealer — let them use the bot as a customer
    console.log(`GroupIngest: dealer ${from} sent normal message, passing to bot flow`);
    return false;
  }

  // It's a car listing — ingest silently
  console.log(`GroupIngest: [DEALER INGEST] from=${from} name=${senderName}`);

  const carData = await _extractCarFromText(msgText, senderName, from);
  if (carData) {
    carData.dealerPhone = from;
    carData.dealerName = senderName;
    inventory.push(carData);
    _saveInventory();
    console.log(`GroupIngest: dealer added ${carData.model || carData.brand || 'car'} — total: ${inventory.length}`);
  }

  return true; // Ingested silently, no reply
}
...
/**
 * Auto-detect car listings from ANY number.
 * No manual number lists needed.
 * If message has 3+ car signals → ingest silently, return true.
 * If it's a normal message → return false, let bot reply.
 *
 * This replaces the need for DEALER_NUMBERS env var.
 * Works for groups forwarded messages, dealer contacts, random sellers.
 */
async function handleAutoIngest(from, msgText, senderName) {
  if (!msgText) return false;

  // Never intercept short messages or obvious customer queries
  if (!_looksLikeCarListing(msgText)) return false;

  // It's a car listing — ingest silently
  console.log(`AutoIngest: [SILENT] car listing detected from ${from} (${senderName})`);
  ...
  if (carData) {
    ...
    inventory.push(carData);
    _saveInventory();
    console.log(`AutoIngest: added ${carData.title || carData.brand || 'car'} — total: ${inventory.length}`);
  }

  return true; // Ingested, no reply
}
