...
// ---------------------------------------------------------
      ...
      if (filteredMatches.length > 0) {
        ...
        if (allMatches.length > 1 && !hasVariantLock) {
          ...
          for (const m of allMatches) {
            ...
            displayVariants.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant });
  ...
  distinct.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant });
...
if (
  ...
  if (variants.length >= 2) {
    ...
    variants.forEach((m, i) => {
      ...
      const varr = String(m.row[m.idxVariant] || '').trim();
      const title = `${mdl} ${varr}`.trim();
      displayVariants2.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant });
