...
function init(config) {
  _config = config || {};
  // Ensure global maps exist on first load (not inside a code path)
...
async function trySmartNewCarIntent(msgText, to) {
  ...
  // ================= PAN-INDIA PROMPT STATE (buttons handle compare/breakup) =================
  // If user is in PAN_INDIA_PROMPT state but types text instead of pressing a button,
  // clear the state and let normal intent logic handle their new query.
  // BUT: do NOT clear if the text is a button title (button presses also set msgText).
  const panSvc = (_config.getLastService(to) || '').toUpperCase();
  if (panSvc === 'PAN_INDIA_PROMPT') {
    // Every wording the webhook maps to those buttons has to be listed here too,
    // or this clears the quote context and the message is answered as a two-car
    // comparison. Belt and braces: the webhook blanks the text as well.
    const btnTitles = /^(yes,?\s*compare|price breakup|breakup|break ?up|on[\s-]?road breakup|breakdown|compare states?|compare|compare state wise|state ?wise|state prices?|pan[\s-]?india|another quote|new quote)$/i;
    if (btnTitles.test((msgText || '').trim())) {
      // This is a button press — let the button handler deal with it, don't clear context
      return false;
    }
    if (global.panIndiaPrompt) global.panIndiaPrompt.delete(to);
    _config.setLastService(to, 'NEW');
    // fall through → normal intent logic handles the typed message
  }
...
// --------------------------------------------------
// SAFE INIT: userNorm + budget (needed before intent guards)
// --------------------------------------------------
let userNorm = normForMatch(t);
if (/\bautomatic\b/.test(t) && !userNorm.includes('at')) {
  userNorm += ' at';
}

// Inline budget parser (also defined in tryQuickNewCarQuote)
function _parseBudget(s) {
  if (!s) return null;
  const norm = String(s).toLowerCase().replace(/[,₹]/g, ' ').replace(/\s+/g, ' ').trim();
  const plainNum = (norm.match(/\b([0-9]{5,9})\b/) || [])[1];
  if (plainNum) { const v = Number(plainNum); if (v > 10000) return v; }
  let m = norm.match(/\b([0-9]+(?:\.[0-9]+)?)\s*(lakh|lac|l|k)\b/);
  if (!m) m = norm.match(/\b([0-9]+(?:\.[0-9]+)?)\s*(l)\b/);
  if (m) { const v = Number(m[1]) * 100000; if (!Number.isNaN(v)) return v; }
  m = norm.match(/\b([0-9]+(?:\.[0-9]+)?)\s*(crore|cr|c)\b/);
  if (m) { const v = Number(m[1]) * 10000000; if (!Number.isNaN(v)) return v; }
  return null;
}

let userBudget = null;
try { userBudget = _parseBudget(t); } catch (e) { userBudget = null; }

// Brand guess (used by model list and other flows)
...
const lastSvc = (_config.getLastService(to) || '').toLowerCase();
    ...
    if (!_config.canSendQuote(to)) {
      await _config.waSendText(to, 'You\u2019ve reached today\u2019s assistance limit for quotes. Please try again tomorrow or provide your details for a personalised quote.');
      return true;
    }
...
if (stateMatch) {
  // State explicitly provided (e.g. UP, HR, HP) — use it for pricing column
  priceCityToken = stateMatch.toUpperCase();
} else if (cityExplicit) {
  try {
...
// 🔒 EXACT TRIM LOCK — answer the trim the customer actually named
// Two faults lived here. A query naming a trim ("hycross vx") was never
// narrowed to that trim, so the highest-scoring Hycross row won whatever its
// badge said. And "VX(O)" contains "VX", so even once narrowed, the dearer (O)
// car could answer a plain VX question — ₹32.4L against ₹30.2L.
{
  const STOPWORDS = /^(the|and|car|new|old|for|in|at|to|of|is|it|my|me|on|or|a|an|price|cost|quote)$/i;
  const trimTokens = (coreTokensArr || []).filter(tk => /^[a-z]{1,4}$/i.test(tk) && !STOPWORDS.test(tk));
  const keyOf = v => String(v || '').toUpperCase().replace(/[^A-Z0-9]/g, '');

  // The model comes first, always. Narrowing by trim alone crossed model lines —
  // "hycross gx" answered with a CRYSTA GX, because that row also carries a GX
  // badge and scored higher.
  const modelTokens = (coreTokensArr || []).filter(tk => tk.length >= 4 && !STOPWORDS.test(tk));
  for (const t of modelTokens) {
    const T = t.toUpperCase();
    const inModel = allMatches.filter(m => keyOf(m.row[m.idxModel]).includes(T));
    if (inModel.length && inModel.length < allMatches.length) {
      console.log(`EXACT_TRIM_LOCK: model "${t}" → ${inModel.length} of ${allMatches.length} row(s)`);
      allMatches = inModel;
    }
  }
  ...
  for (const t of trimTokens) {
    ...
    const T = t.toUpperCase();
    // Word boundaries, so "E" matches "GLANZA E MT" but not "7 SEATER".
    const wordRe = new RegExp('\\b' + T + '\\b', 'i');
    let withTrim = allMatches.filter(m => wordRe.test(String(m.row[m.idxVariant] || '')));
    // "vx(o)" arrives as "vxo" once punctuation is stripped, so fall back to a
    // punctuation-free comparison before giving up on the token.
    if (!withTrim.length) withTrim = allMatches.filter(m => keyOf(m.row[m.idxVariant]).includes(T));
    if (withTrim.length && withTrim.length < allMatches.length) {
      console.log(`EXACT_TRIM_LOCK: "${t}" → ${withTrim.length} of ${allMatches.length} row(s)`);
      allMatches = withTrim;
    }
  }

  const askedForO = /\(\s*o\s*\)|\b[a-z]{1,3}\s*o\b|\b[a-z]{1,4}o\b/i.test(canonicalUserNorm);
  if (!askedForO && trimTokens.length && allMatches.length) {
    const oMarker = t => new RegExp('\\b' + t + '\\s*(\\(\\s*o\\s*\\)|o\\b)', 'i');
    const plainOnly = allMatches.filter(m => {
      const v = String(m.row[m.idxVariant] || '');
      return !trimTokens.some(t => oMarker(t).test(v));
    });
    if (plainOnly.length && plainOnly.length < allMatches.length) {
      console.log(`EXACT_TRIM_LOCK: dropped ${allMatches.length - plainOnly.length} (O) row(s)`);
      allMatches = plainOnly;
    }
  }
}
...
// ---------------------------------------------------------
      ...
      if (filteredMatches.length > 0) {
        ...
        if (allMatches.length > 1 && !hasVariantLock) {
          ...
          for (const m of allMatches) {
            ...
            displayVariants.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant, exShow: m.exShow || 0, fuel: m.fuel || '', header: tables[m.brand]?.header || [] });
            out.push(`${displayVariants.length}) *${mdl} ${varr}* – On-road ₹ ${fmtMoney(m.onroad)}`);
          }
          if (displayVariants.length > 1) {
            out.push('');
            out.push('Reply with the *number* (1–25) to get the detailed on-road price & offers for that variant.');
            if (!global.lastVariantList) global.lastVariantList = new Map();
            global.lastVariantList.set(to, { ts: Date.now(), variants: displayVariants, stateMatch: stateMatch || null });
          }
          await _config.waSendText(to, out.join("\n"));
          _config.setLastService(to, 'NEW');
          return true;
  ...
  distinct.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant, exShow: m.exShow || 0, fuel: m.fuel || '', header: tables[m.brand]?.header || [] });
...
if (
  ...
  if (variants.length >= 2) {
    ...
    const displayVariants2 = [];
    variants.forEach((m, i) => {
      const mdl = String(m.row[m.idxModel] || '').trim();
      const varr = String(m.row[m.idxVariant] || '').trim();
      const title = `${mdl} ${varr}`.trim();
      displayVariants2.push({ title, onroad: m.onroad || 0, brand: m.brand, score: m.score || 0, row: m.row, idxModel: m.idxModel, idxVariant: m.idxVariant, exShow: m.exShow || 0, fuel: m.fuel || '', header: tables[m.brand]?.header || [] });
    ...
    if (!global.lastVariantList) global.lastVariantList = new Map();
    global.lastVariantList.set(to, { ts: Date.now(), variants: displayVariants2, stateMatch: stateMatch || null });
...
if (
  ...
  console.log('SINGLE_QUOTE_DEBUG:', { stateMatch, priceCityToken, brand: best.brand, onroad: best.onroad, exShow: best.exShow, model: best.row?.[best.idxModel], variant: best.row?.[best.idxVariant] });

  // A price list forwarded on WhatsApp overrides the sheet — applied before the
  // loan amount and EMI are derived so every figure below matches.
  let benefitLine = null;
  try {
    if (_config.priceSync) {
      const ovModel = best.idxModel >= 0 ? String(best.row[best.idxModel] || '') : '';
      const ovVariant = best.idxVariant >= 0 ? String(best.row[best.idxVariant] || '') : '';
      const ov = _config.priceSync.lookup(best.brand || '', ovModel, ovVariant);
      if (ov) {
        const prevEx = Number(String(best.exShow || '').replace(/[,₹\s]/g, '')) || 0;
        if (ov.exShowroom) best.exShow = ov.exShowroom;
        if (ov.onRoad) best.onroad = ov.onRoad;
        else if (ov.exShowroom && prevEx) {
          const prevOnRoad = Number(String(best.onroad || '').replace(/[,₹\s]/g, '')) || 0;
          if (prevOnRoad) best.onroad = prevOnRoad - prevEx + ov.exShowroom;
        }
        benefitLine = _config.priceSync.discountLine(ov);
        console.log('PRICE_OVERRIDE_APPLIED:', { model: ovModel, variant: ovVariant, exShow: best.exShow, onroad: best.onroad });
      }
    }
  } catch (e) {
    console.warn('price override skipped:', e && e.message ? e.message : e);
  }
  // The discount line is part of every quote, stated figure or not.
  if (!benefitLine && _config.priceSync && typeof _config.priceSync.discountLine === 'function') {
    try { benefitLine = _config.priceSync.discountLine(null); } catch (e) {}
  }
  ...
  0;
  const roi = _config.getNewCarROI ? _config.getNewCarROI(loanAmt) : Number(process.env.NEW_CAR_ROI || 8.1);
...
lines.push(
  ...
  if (benefitLine) lines.push(benefitLine);
...
try {
  ...
  // OPTION 3 — FLEX PAY / BALLOON (same engine as the deal cards and the website)
  try {
    if (_config.inventory && typeof _config.inventory.flexPayLines === 'function') {
      const flex = _config.inventory.flexPayLines({ amount: loanAmt, roi, months: 60, used: false });
      if (flex && flex.length) {
        lines.push('');
        flex.forEach(l => lines.push(l));
      }
    }
  } catch (e) {
    if (_config.DEBUG) console.warn('NEW CAR FLEX PAY ERROR:', e?.message);
  }
...
if (typeof _config.noteLeadInterest === 'function') {
  _config.noteLeadInterest(to, `Quoted: ${best.brand} ${mdl} ${varr}`);
}
...
// Send interactive buttons (Compare / Price Breakup / Another Quote)
...
// ---------------- END SPEC SHEET ----------------
    // The single-quote branch builds `lines`, sends it and returns above. That
    // branch is guarded by !userBudget, so a message stating a budget — "Price 19
    // lakh" — skipped it, left `lines` undeclared, and this line threw
    // ReferenceError and killed the whole quote. Reaching here means nothing was
    // composed, so hand back to the caller and let the other handlers try.
    if (typeof lines === 'undefined' || !Array.isArray(lines) || !lines.length) {
      console.warn('tryQuickNewCarQuote: nothing composed for', JSON.stringify(String(msgText || '').slice(0, 60)));
      return false;
    }
    await _config.waSendText(to, lines.join('\n'));
    await _config.sendNewCarButtons(to);
