...
async function trySmartNewCarIntent(msgText, to) {
  ...
  if (panSvc === 'PAN_INDIA_PROMPT') {
    ...
    const btnTitles = /^(price breakup|breakup|break ?up|on[\s-]?road breakup|breakdown|compare states?|compare|compare state wise|state ?wise|state prices?|pan[\s-]?india|another quote|new quote)$/i;
